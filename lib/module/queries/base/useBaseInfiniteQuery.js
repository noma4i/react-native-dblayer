...
export const useBaseInfiniteQuery = config => {
  ...
  const skipInitialFetch = useMemo(() => isInactive ? false : config.collection.shouldSkipInitialFetch(filter, config.staleTime), [config.collection, config.staleTime, filter, isInactive]);
