 ...
 */
export const executeDbInfiniteRequest = async (config, pageParam, patchState) => {
  ...
  const contract = config.resolveSyncContract ? config.resolveSyncContract({
    ...
    nodes,
    scope: modelFilter
  }) : pageParam === undefined ? replaceSyncContract('initial', modelFilter) : mergeSyncContract('loadMore', modelFilter);
