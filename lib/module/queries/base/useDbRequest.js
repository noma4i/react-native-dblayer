...
export const useDbInfiniteRequest = config => {
  ...
  }, [config.direction, config.emptyStaleTime, config.enabled, config.gcTime, config.getCursor, keySignature, config.maxPages, config.query, config.read, config.readMode, config.refetchOnMount, config.resolveSyncContract, config.scope, config.staleTime]);
