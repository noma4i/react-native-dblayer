...
export const useBaseQuery = config => {
  ...
  const skipInitialFetch = useMemo(() => isInactive ? false : resolveSkipInitialFetch(config.collection, config.staleTime), [config.collection, config.staleTime, isInactive]);
