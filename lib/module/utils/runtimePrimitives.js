...
import { readShapeOrThrow } from "../schema/shape.js";
 ...
 */
export const createKeyedBatchBuffer = config => {
  const buckets = new Map();
  let generation = null;
  const isCurrentGeneration = () => generation == null || generation === getRuntimeGeneration();
  ...
  const flushBucket = (key, bucket) => {
    ...
    try {
      ...
      getDbLogger().error('db', 'keyed batch buffer flush failed', {
        key,
        error
      });
    }
 ...
 */
export const createKeyedArrayPatcher = (shape, options) => ({
  upsert(rows, input) {
    const next = readShapeOrThrow(shape, input, 'Keyed array patch item');
 ...
 */
export const createIdArrayPatcher = () => ({
