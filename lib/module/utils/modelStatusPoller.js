...
import { createGenerationFence, createSingleFlight } from "./runtimePrimitives.js";
const IDLE_PHASE = {
  phase: 'idle',
  attempts: 0
};
...
export const createModelStatusPoller = config => {
  ...
  const emit = id => {
    for (const subscriber of subscribers.get(id) ?? []) {
      try {
        ...
        getDbLogger().error('ModelStatusPoller', 'phase subscriber failed', {
          id,
          error
        });
      }
   ...
   */
  const runFetch = async (id, session) => {
  ...
  return {
    ...
    getPhase,
    usePhase(id) {
      const subscribeToId = useCallback(listener => subscribe(id, listener), [id]);
