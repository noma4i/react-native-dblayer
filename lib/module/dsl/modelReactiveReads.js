...
import { pickLowestRow } from "../core/ordering.js";
...
export const createModelReactiveReads = options => {
  ...
  return {
    ...
    first: (where, readOptions = {}) => {
      ...
      return useModelQuery(options.modelId, signature, spec, rows => rows[0] ? gateRef.current.project(rows[0], optionsRef.current) : undefined);
    },
    ...
    related: (id, relationName, readOptions = {}) => {
      ...
      if (relation.kind === 'belongsTo') {
        ...
        compute = () => {
          ...
          return pickLowestRow(relation.model.where({
            [relation.foreignKey]: id
          }), comparator);
        };
