...
export const defineMutation = config => {
  ...
  const planFromRespond = (data, context, optimistic, input) => {
    ...
    if (node != null) {
      ...
      if (context.tempId && id !== context.tempId && optimistic.model.get(context.tempId) !== undefined) ops.push(...getInternalModelHandle(optimistic.model).planReplace(context.tempId, row));else ops.push(...getInternalModelHandle(optimistic.model).planRows([row]));
  ...
  const inverseFromRespond = (data, context, optimistic) => {
    ...
    for (const sink of config.extract?.({
      ...
      for (const row of sink.rows) if (isRecord(row) && typeof row.id === 'string') targets.push({
        model,
        id: row.id
      });
    }
    ...
    try {
      ...
      if (optimistic && isRespondOptimistic(optimistic) && insertedTempId) {
        ...
        if (optimistic.failure === 'rollback') {
          if (insertedTempId) getApplyRuntime().apply(expandPlan([{
            kind: 'destroy',
            model: optimistic.model.modelId,
            ids: [insertedTempId],
            tombstone: false
          ...
          if (patch) optimistic.model.patch(tempId, patch);
  ...
  const retry = async tempId => {
    ...
    if (patch) optimisticConfig.model.patch(tempId, patch);
