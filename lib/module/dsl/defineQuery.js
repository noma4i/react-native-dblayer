...
export const defineQuery = config => {
  ...
  const committedRowsKey = scopeKey => compositeKey(keyName, scopeKey);
  const staleGuardKey = (scope, scopeKey) => isScopeDestination(config.into) ? compositeKey(String(config.into.modelId ?? keyName), getInternalScopeHandle(config.into).key(scope)) : compositeKey(keyName, scopeKey);
  ...
  const committedIdsOf = rows => rows.flatMap(row => isRecord(row) && row.id != null ? [String(row.id)] : []);
  ...
  const applyResponse = (scope, data, resetOrder, resurrectDestroyed) => {
    ...
    if (isScopeDestination(config.into)) {
      ...
      ops.push(...getInternalScopeHandle(config.into).planApply(scope, scopeRows, coverage, {
        resetOrder
      }));
      ...
      ops.push(...getInternalModelHandle(config.into).planRows(nodes, resurrectDestroyed ? {
        origin: 'event'
      } : undefined));
