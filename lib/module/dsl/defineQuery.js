...
export const defineQuery = config => {
  ...
  const applyResponse = (scope, data, resetOrder, resurrectDestroyed) => {
    ...
    if (isScopeDestination(config.into))
      ops.push(
        ...getInternalScopeHandle(config.into).planApply(scope, rows, coverage, {
          resetOrder
        })
      );
    else
      ops.push(
        ...getInternalModelHandle(config.into).planRows(
          nodes,
          resurrectDestroyed
            ? {
                origin: 'event'
  ...
  const execute = async (scope, key, resurrectDestroyed, context) => {
    ...
    try {
      data = responseDataOrThrow(
        await getDbRuntimeConfig().transport.query({
  ...
  const useReader = (scope, enabled, resurrectDestroyed, forceAbsentRefetch) => {
    ...
    useEffect(() => {
      ...
      const unsubscribeOnline = subscribeFetchNetwork(() => {
        ...
        if (!isQueryFresh(client, queryKey, staleTimeOf(key)))
          void run(scope, {
            restart: false,
            resurrectDestroyed
          }).catch(() => {});
