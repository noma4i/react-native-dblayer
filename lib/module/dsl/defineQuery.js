...
import { getApplyRuntime, getDbRuntimeConfig, getRuntimeGeneration } from './configure.js';
...
export const defineQuery = config => {
  ...
  const applyResponse = (scope, data, resetOrder) => {
    ...
    if (isScopeDestination(config.into)) {
      ...
      ops.push(
        ...(config.into.__planApply?.(scope, scopeRows, coverage, {
          resetOrder
        }) ?? [])
      );
    } else {
  ...
  const invalidate = scope => {
    ...
    for (const registeredScope of registeredScopes.values()) {
      if (matchesPartialScope(registeredScope, scope))
        void client.invalidateQueries({
