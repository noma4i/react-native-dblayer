...
import { bridgeWindowPagination } from "./pagination.js";
...
export const defineQuery = rawConfig => {
  ...
  const staleTimeOf = key => {
    ...
    return meta?.lastCount === 0 && (config.emptyStaleTime ?? defaults.emptyStaleTime) != null ? config.emptyStaleTime ?? defaults.emptyStaleTime : resolveStaleTime(config.staleTime, defaults) ?? defaults.staleTime ?? 0;
  };
