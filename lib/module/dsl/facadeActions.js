...
import { registerOptimisticInsertPlanner } from "./optimisticInsertPlanners.js";
...
import { planRequestFailureRollback } from "../core/requestRollback.js";
...
export const createAction = (runtime, name, definition) => {
  ...
  if (requestDefinition.optimistic && modelRootIntentOf(requestDefinition.optimistic.root) === 'insert') {
    registerOptimisticInsertPlanner(runtime.modelId, actionKey, (input, tempId, operationId) => buildOptimisticPlan(input, tempId, operationId, false).ops);
  }
    ...
    try {
      ...
      if (active) {
        if (error instanceof MutationDeliveryUnknownError) {
          ...
          const planned = planRequestFailureRollback(active, id => getInternalModelHandle(runtime).readRow(id), (row, memberships) => getInternalModelHandle(runtime).planRestore(row, memberships));
