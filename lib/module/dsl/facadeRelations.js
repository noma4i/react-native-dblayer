 ...
 */
export const localLoadingState = hasData => ({
  phase: 'ready',
  hasData,
  isReady: true,
  showSkeleton: false,
...
export const createLocalResult = (data, hasData, hasMore, loadMore) => ({
  data,
...
export const createNamedRelation = (runtime, name, params, query, remoteType) => {
...
export const createWhereRelation = (runtime, where, options) => ({
...
export const createByIdsRelation = (runtime, ids) => ({
  read: () => (ids ?? []).flatMap(id => {
  ...
  seed: rows => runtime.seed(rows),
  use: useOptions => {
    const rows = runtime.use.byIds(ids, useOptions?.renderKeys ? {
      renderKeys: useOptions.renderKeys
    } : {}).rows;
    return createLocalResult(rows, rows.length > 0, false, () => {});
  },
...
export const createAssociationRelation = (runtime, name, id) => {
