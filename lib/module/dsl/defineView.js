...
import { noteFkIndex } from "../core/diagnostics.js";
...
import { compareRowsBySpec, pickLowestRow } from "../core/ordering.js";
...
import { getCommitBus, getDbRuntimeConfig } from "./configure.js";
import { getInternalModelHandle, getInternalScopeHandle } from "../core/internalHandles.js";
const foreignKeyIndexes = new Map();
const foreignKeyIndexKey = (targetModelId, foreignKey) => compositeKey(targetModelId, foreignKey);
...
export const defineView = (model, name, publicConfig) => {
  ...
  const useItems = (scopeValue, limit) => {
    ...
    const includeProjectionGatesRef = useRef(new Map());
    ...
    const evaluate = () => {
      ...
      const projected = visibleRows.map((row, index) => {
        ...
        for (const [alias, include] of Object.entries(config.include)) {
          ...
          if (relationName) {
            ...
            included[alias] = Array.isArray(resolved) ? resolved.filter(candidate => hasRequiredFields(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys)) : hasRequiredFields(resolved, required ?? []) ? projectIncludedRow(alias, resolved, includeRenderKeys) : null;
            if (relation.kind === 'belongsTo') {
              const id = row[relation.foreignKey];
            ...
            const resolved = ids.map(id => target.find(id)).filter(candidate => hasRequiredFields(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys));
        ...
        if (current && current.row === row && rowsShallowEqual(current.included, included)) return {
          row,
      ...
      if (config.sort) {
        const compare = compareRowsBySpec(config.sort);
    ...
    return useLiveRead(() => {
      ...
      return snapshot;
    }, evaluatedRef.current.deps, (left, right) => left.resolved === right.resolved && left.totalCount === right.totalCount && arraysShallowEqual(left.items, right.items), compositeKey(scopeKey ?? '', limit === null ? 'all' : String(limit)));
  };
  return {
