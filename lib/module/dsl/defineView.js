      ...
      const items = visibleRows.map((row, index) => {
        ...
        for (const [alias, include] of Object.entries(config.include)) {
          ...
          if (relationName) {
            ...
            const resolved = ids.map(id => target.get(id)).filter(candidate => hasRequiredFields(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys));
