...
import { buildScopeKey, isWhereOperatorValue, matchesDbWhere } from "../core/compileDbWhere.js";
...
import { coldResetModel, CorruptionError } from "../core/recovery.js";
...
import { createEntityState } from "../core/planes/entityState.js";
...
import { noteReplaceRejected } from "../core/diagnostics.js";
...
import { useLiveRead, arraysShallowEqual, rowsShallowEqual } from "../read/useLiveRead.js";
...
import { getApplyRuntime, getCommitBus, getDbRuntimeConfig, getOperationState, getStoragePrefix, hasReplayedJournal } from "./configure.js";
import { defineFetch } from "./defineFetch.js";
import { clearFailedOptimisticMutation, defineMutation } from "./defineMutation.js";
...
import { isRecord, stringifyNullish } from "../utils/normalizeHelpers.js";
...
import { registerInternalModelHandle, registerInternalScopeHandle } from "../core/internalHandles.js";
...
const sortRowsBySpec = (rows, sort) => 'comparator' in sort ? [...rows].sort(sort.comparator) : sortModelReadRows(rows, [{
...
export const defineModel = config => {
    ...
    return (previous, incoming, ctx) => {
      ...
      for (const group of groups ?? []) {
        ...
        for (const field of group.fields) {
          ...
          if (value === undefined) effective[field] = previous[field];else effective[field] = value;
        }
      }
      return effective;
    };
  ...
  const planes = () => {
    ...
    try {
      ...
      getDbLogger().error('cold-model recovery', {
        model: config.id,
        keyClass: error.keyClass,
        key: error.storageKey
      });
      entityState.hydrate();
      scopeIndex.hydrate();
    }
  ...
  const membershipForUpsert = (before, after) => {
    ...
    for (const [scopeName, spec] of membershipScopes) {
      const beforeValue = before && matchesMemberPredicate(spec, before) ? scopeValueFromRow(spec.by, before) : null;
      const afterValue = matchesMemberPredicate(spec, after) ? scopeValueFromRow(spec.by, after) : null;
  ...
  const writeRows = (rows, origin, mergeBase, operationId) => {
    const changes = [];
    for (const value of rows) {
      ...
      let gatedIncoming = config.write ? incoming : {
        ...current,
        ...incoming
      };
      const result = planes().entityState.upsert(gatedIncoming, {
  ...
  const applyTarget = {
    ...
    upsert: writeRows,
    patch: (id, patch, operationId) => {
      const key = String(id);
      const current = planes().entityState.read(key);
      if (!current) return null;
      const row = config.write ? {
        ...patch,
        id: key
      } : {
        ...current,
      ...
      const result = planes().entityState.upsert(row, {
        ctx: {
          origin: 'patch',
          operationId
        }
  ...
  registerSchemaDeclaration({
    id: config.id,
    name: config.name,
    fields: Object.fromEntries(Object.entries(config.fields).map(([name, field]) => [name, {
      kind: field.kind,
      mode: field.mode,
      hasDefault: field.hasDefault
    }])),
    scopes: Object.fromEntries(Object.entries(config.scopes ?? {}).map(([name, spec]) => {
  ...
  const applyEvent = ops => {
    getApplyRuntime().apply(ops.map(op => op.kind === 'upsert' && op.origin === undefined ? {
      kind: 'upsert',
      model: op.model,
      rows: op.rows,
      origin: 'event'
  ...
  function whereRead(where) {
    ...
    return createReadBuilder(where, {
      ...
      pluck: (criteria, orders, limit, required, projection, field) => {
        ...
        const projectionRef = useRef(projection);
        projectionRef.current = projection;
        const signature = incrementalSignature('where-pluck', config.id, buildScopeKey({
          criteria,
          orders: effectiveOrders,
          limit,
          required,
      ...
      exists: (criteria, required) => {
        const signature = incrementalSignature('where-exists', config.id, buildScopeKey({
          criteria,
          required
        }));
        return useIncrementalRead({
  ...
  const makeScopeHandle = scopeName => {
    ...
    const readScopeRows = (scopeValue, options = {}) => {
      ...
      return useScopeLiveRows(config.id, scopeKey, applyTarget.scopeSortMeta(scopeKey ?? `${scopeName}:`), () => scopeKey == null || planes().scopeIndex.read(scopeKey).generation > 0, options);
    };
    const scopeHandle = {
      modelId: config.id,
      use: readScopeRows,
      useFirst: (scopeValue, options = {}) => readScopeRows(scopeValue, options)[0],
      useWindow: (scopeValue, options = {}) => {
        ...
        useScopeAccess(scopeKey);
        const window = useScopeLiveWindowRows(config.id, scopeKey, applyTarget.scopeSortMeta(scopeKey ?? `${scopeName}:`), windowSize, () => scopeKey == null || planes().scopeIndex.read(scopeKey).generation > 0, options);
        return {
          rows: window.rows,
  ...
  const model = {
    ...
    insert: row => applyEvent([{
      kind: 'upsert',
      model: config.id,
      rows: [row]
    }]),
    insertMany: rows => applyEvent([{
      kind: 'upsert',
      model: config.id,
      rows
    }]),
    ...
    use: {
      ...
      find: (id, options = {}) => {
        ...
        }, key == null ? [] : [rowDep(key, required.length > 0 ? required : undefined)], options, `${config.id}.use.find`);
      },
      field: (id, field) => {
        const key = id == null ? undefined : String(id);
      ...
      byIds: (ids, options = {}) => {
        ...
        }, resolvedIds.map(id => rowDep(id)), Object.is);
      },
  ...
  registerInternalModelHandle(model, {
    ...
    applyPatch: (id, patch, operationId) => getApplyRuntime().apply([{
      kind: 'patch',
      model: config.id,
      id: String(id),
      patch,
