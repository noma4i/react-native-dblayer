...
export const defineModel = config => {
  ...
  const keyForScope = (scopeName, scopeValue) => `${scopeName}:${buildScopeKey(coerceScopeValueForKey(scopeName, scopeValue))}`;
  ...
  const membershipForUpsert = row => {
    ...
    for (const [scopeName, spec] of membershipScopes) {
      ...
      const afterValue = matchesMemberPredicate(spec, merged) ? scopeValueFromRow(spec.by, merged) : null;
  ...
  const model = {
    ...
    use: {
      pending: id => {
        ...
        const readPending = useCallback(() => key != null && getOperationState().pending().some(operation => operation.model === config.id && (operation.rowIds ?? operation.tempIds).includes(key)), [key]);
      ...
      byIds: (ids, options = {}) => {
        ...
        const rows = useProjectedLiveRows(() => resolvedIds.map(id => planes().entityState.read(id)).filter(row => row !== undefined), resolvedIds.map(id => rowDep(id)), options, `${config.id}.use.byIds`);
