...
export const defineModel = config => {
  ...
  const makeScopeHandle = scopeName => {
    ...
    return {
      ...
      useCount: scopeValue => useLiveRead(() => scopeValue == null ? 0 : planes().scopeIndex.read(keyForScope(scopeValue)).entries.length, scopeValue == null ? [] : [scopeDep(keyForScope(scopeValue))]),
