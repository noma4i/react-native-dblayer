...
export const defineModel = config => {
  ...
  const model = {
    ...
    use: {
      pending: id => {
        const readPending = useCallback(() => id != null && getOperationState().pending().some(operation => operation.model === config.id && (operation.rowIds ?? operation.tempIds).includes(id)), [id]);
