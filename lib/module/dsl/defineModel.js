...
const keyForScope = scopeValue => buildScopeKey(scopeValue);
const EMPTY_ROWS = [];
...
export const defineModel = config => {
  ...
  const makeScopeHandle = scopeName => {
    ...
    return {
      modelId: config.id,
      use: scopeValue => {
        const rows = useLiveRead(() => scopeValue == null ? EMPTY_ROWS : scopeSortedRows(scopeName, scopeValue), scopeValue == null ? [modelDep] : memberDeps(keyForScope(scopeValue), planes().scopeIndex.read(keyForScope(scopeValue)).entries), arraysShallowEqual);
        return rows;
      },
      ...
      useCount: scopeValue => useLiveRead(() => scopeValue == null ? 0 : planes().scopeIndex.read(keyForScope(scopeValue)).entries.length, scopeValue == null ? [modelDep] : [scopeDep(keyForScope(scopeValue))]),
