...
import { useScopeReadRows, useScopeReadWindowRows } from "../read/scopeReadEngine.js";
import { useLiveRead } from "../read/useLiveRead.js";
import { useRef, useState } from 'react';
import { getDbRuntimeConfig } from "./configure.js";
import { sortRowsBySpec } from "./modelReadAccess.js";
  ...
  return scopeName => {
    ...
    const scopeHandle = {
      ...
      useCount: scopeValue => {
        const scopeKey = scopeValue === null ? null : options.keyForScope(scopeName, scopeValue);
        options.useScopeAccess(scopeKey);
        return useLiveRead(() => scopeValue === null ? 0 : planes().scopeIndex.read(options.keyForScope(scopeName, scopeValue)).entries.length, scopeKey == null ? [] : [options.scopeDep(scopeKey)]);
      },
