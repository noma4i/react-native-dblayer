...
import { getDbRuntimeConfig, getOperationState } from "./configure.js";
  ...
  return scopeName => {
    ...
    const planApply = (scopeValue, rows, coverage, planOptions) => {
      const liveRows = rows.filter(({
        row
      }) => options.admitPlanRow(row) !== undefined).filter(({
        row
    ...
    const scopeHandle = {
      ...
      seed: (scopeValue, rows) => {
        const liveRows = rows.filter(row => options.admitPlanRow(row) !== undefined).filter(row => !planes().entityState.isTombstoned(options.normalize(row).id)).map(row => ({
          row: row
        }));
