...
import { rowsShallowEqual, useLiveRead } from "./useLiveRead.js";
...
export const validateProjectionOptions = (options, surface, validation) => {
...
export const createProjectionGate = () => {
  ...
  return {
    ...
    projectRows(rows, options) {
      if (previousSource === rows && previousOptionsSignature !== null && previousOptionsSignature.select === options.select && (previousOptionsSignature.renderKeys === options.renderKeys || arraysShallowEqual(previousOptionsSignature.renderKeys ?? [], options.renderKeys ?? []))) {
        return previousRows;
      }
