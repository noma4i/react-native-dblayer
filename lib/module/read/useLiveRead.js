...
const depsSignature = deps => deps.map(dep => dep.kind === 'model' ? compositeKey('m', dep.model) : dep.kind === 'scope' ? compositeKey('s', dep.model, dep.scopeKey) : dep.kind === 'pending' ? compositeKey('p', dep.model, dep.id) : compositeKey('r', dep.model, dep.id, dep.fields?.join(',') ?? '')).join('|');
