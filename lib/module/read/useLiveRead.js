...
const depsSignature = deps => compositeKey(...deps.map(dep => dep.kind === 'model' ? compositeKey('m', dep.model) : dep.kind === 'scope' ? compositeKey('s', dep.model, dep.scopeKey) : dep.kind === 'pending' ? compositeKey('p', dep.model, dep.id) : compositeKey('r', dep.model, dep.id, semanticValue(dep.fields ?? []))));
 ...
 */
export const useLiveRead = (compute, deps, isEqual = Object.is, inputSignature = '') => {
  ...
  if (stateRef.current === null) {
    stateRef.current = {
      value: compute(),
      version: 0,
      signature: compositeKey(inputSignature, depsSignature(deps)),
      compute,
      isEqual,
      deps
    };
