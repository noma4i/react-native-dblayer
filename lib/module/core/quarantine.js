...
import { noteDataLoss, noteQuarantinePut } from "./diagnostics.js";
...
const isQuarantineEntry = value => isNonArrayRecord(value) && (value.kind === 'ledger' || value.kind === 'operation' || value.kind === 'row') && typeof value.model === 'string' && isNonEmptyString(value.id) && isNonEmptyString(value.reason) && Object.hasOwn(value, 'raw');
const isQuarantineState = value => isNonArrayRecord(value) && Array.isArray(value.entries) && value.entries.every(isQuarantineEntry);
const readState = () => {
  ...
  getDbLogger().error('quarantine state unreadable, restarting empty', {
    kind: decoded.kind
  });
  return {
    entries: []
...
export const putQuarantine = entry => {
  ...
  const diagnosticTickets = state.entries.filter(candidate => candidate.reason === 'plan-row-rejected');
  ...
  writeState(state);
  noteQuarantinePut();
  reportSyncError(new Error(`quarantined ${entry.kind} ${entry.model}/${entry.id}: ${entry.reason}`), {
    source: 'quarantine',
    model: entry.model,
    key: entry.id
  }, 'quarantine');
...
export const readQuarantineEntries = () => readState().entries.map(entry => ({
  ...entry
}));

/** Remove and return the entries the predicate accepts - the only automatic removal path (fsck restore). */
export const takeQuarantineEntries = accepts => {
  const state = readState();
