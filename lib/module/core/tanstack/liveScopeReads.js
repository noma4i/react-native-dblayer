 ...
 */
export function useScopeLiveRows(modelId, scopeKey, sortMeta, options = {}) {
  ...
  const getSnapshot = useCallback(() => gateRef.current.projectRows(scopeKey == null ? EMPTY_ROWS : entryFor(modelId, scopeKey, sortMeta).snapshot, optionsRef.current), [modelId, scopeKey, sortMeta]);
 ...
 */
export function useScopeLiveWindowRows(modelId, scopeKey, sortMeta, windowSize, options = {}) {
