...
import { getApplyTarget } from "../apply/transaction.js";
import { ensureModelCollection, ensureMembershipCollection, membershipWriterFor, runInWriteBatch, writerFor } from "./facade.js";
...
export function startCollectionMirror(bus) {
    ...
    runInWriteBatch(() => {
      for (const modelId of new Set([...rowIdsByModel.keys(), ...scopeKeysByModel.keys()])) {
        ...
        for (const id of rowIds) {
          ...
          if (!rowsShallowEqual(current, next)) {
            writer.write({
              type: `update`,
              value: next
            });
        ...
        for (const scopeKey of scopeKeys) {
          ...
          if (meta.kind === `field`) {
            if (structural.rebuild) {
              ...
              for (const row of expected) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: row
                });else if (!rowsShallowEqual(current, row)) membershipWriter.write({
                  type: `update`,
                  value: row
                });
              }
              ...
              for (const rowId of structural.appendIds) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: next
                });else if (!rowsShallowEqual(current, next)) membershipWriter.write({
                  type: `update`,
                  value: next
                });
              }
            ...
            for (const change of batch.rows) {
              ...
              if (!rowsShallowEqual(current, next)) membershipWriter.write({
                type: `update`,
                value: next
              });
            }
          ...
          const expected = target.readScopeEntries(scopeKey).map(entry => ({
            key: `${scopeKey}\0${entry.id}`,
            scopeKey,
            rowId: entry.id,
            seq: entry.order
          }));
...
export function seedCollections(models) {
  runInWriteBatch(() => {
