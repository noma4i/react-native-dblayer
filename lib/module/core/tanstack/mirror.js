...
const hasChanged = (current, next) => {
  ...
  return Object.keys({
    ...currentRecord,
    ...nextRecord
  }).some(key => currentRecord[key] !== nextRecord[key]);
};
        ...
        for (const scopeKey of scopeKeys) {
          ...
          if (meta.kind === `field`) {
            if (structural.rebuild) {
              ...
              for (const row of expected) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: row
                });else if (hasChanged(current, row)) membershipWriter.write({
                  type: `update`,
                  value: row
                });
              }
              ...
              for (const rowId of structural.appendIds) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: next
                });else if (hasChanged(current, next)) membershipWriter.write({
                  type: `update`,
                  value: next
                });
              }
            ...
            for (const change of batch.rows) {
              ...
              if (hasChanged(current, next)) membershipWriter.write({
                type: `update`,
                value: next
              });
            }
