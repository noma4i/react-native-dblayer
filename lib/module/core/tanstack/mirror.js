...
const rowsDiffer = (current, next) => !rowsShallowEqual(current, next);
    ...
    runInWriteBatch(() => {
      for (const modelId of new Set([...rowIdsByModel.keys(), ...scopeKeysByModel.keys()])) {
        ...
        for (const scopeKey of scopeKeys) {
          ...
          if (meta.kind === `field`) {
            if (structural.rebuild) {
              ...
              for (const row of expected) {
                ...
                else if (rowsDiffer(current, row))
                  membershipWriter.write({
                    type: `update`,
                    value: row
                  });
              ...
              for (const rowId of structural.detachIds)
                membershipWriter.write({
                  type: `delete`,
                  key: `${scopeKey}\0${rowId}`
                });
              for (const rowId of structural.appendIds) {
                ...
                else if (rowsDiffer(current, next))
                  membershipWriter.write({
                    type: `update`,
                    value: next
                  });
          ...
          const expected =
            meta.kind === `comparator`
              ? target.readScopeOrder(scopeKey).map((rowId, seq) => ({
                  key: `${scopeKey}\0${rowId}`,
                  scopeKey,
                  rowId,
                  seq
