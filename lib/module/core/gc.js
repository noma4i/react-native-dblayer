...
export const collectGarbage = () => {
  ...
  const activeScopeDependencies = new Set(getCommitBus().activeDependencies().filter(dependency => dependency.kind === 'scope').map(dependency => `${dependency.model}\0${dependency.scopeKey}`));
  const now = Date.now();
