...
import { compareCodepoints, compositeKey, compositeStorageKey, parseCompositeKey } from "../serialize.js";
import { keysForSequence } from "../orderKey.js";
...
const isScopeIndexValue = value => isRecord(value) && typeof value.generation === 'number' && (value.coverage === 'delta' || value.coverage === 'page' || value.coverage === 'complete') && Array.isArray(value.entries) && value.entries.every(entry => isRecord(entry) && typeof entry.id === 'string' && typeof entry.orderKey === 'string' && (entry.edge === undefined || isRecord(entry.edge)));
...
export const createScopeIndex = options => {
  ...
  const scopeEntry = (id, orderKey, edge) => ({
    id,
    orderKey,
    ...(edge ? {
      edge
  ...
  const reconcileNext = (key, coverage, incoming, opts) => {
    ...
    for (const row of incoming) {
      ...
      if (row.orderKey !== undefined) keyed.push(scopeEntry(row.id, row.orderKey, row.edge ?? existing?.edge));else if (existing) keyed.push(scopeEntry(existing.id, existing.orderKey, row.edge ?? existing.edge));else keyless.push(row);
    }
