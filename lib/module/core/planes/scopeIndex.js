...
export const isScopeEntry = value => isNonArrayRecord(value) && isNonEmptyString(value.id) && isOrderKey(value.orderKey) && (value.edge === undefined || isNonArrayRecord(value.edge));
...
export const createScopeIndex = options => {
  ...
  const reconcileNext = (key, coverage, incoming, opts) => {
    ...
    if (coverage === 'complete') {
      ...
      if (arraysShallowEqual(previous.entries, deduplicated.map(row => row.id), (entry, id) => entry.id === id)) {
