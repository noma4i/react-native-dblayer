...
const isPersistedOperationState = value => isNonArrayRecord(value) && value.recordVersion === OPERATION_STATE_RECORD_VERSION && isOperationRecordMap(value.operations) && Array.isArray(value.committedKeys) && value.committedKeys.every(isNonEmptyString);
...
export const readCommittedOnceKeys = (storage, prefix) => {
  ...
  if (rawOperations) {
    const state = decodeSupportedPersistence(rawOperations, PERSISTENCE_SCHEMA_VERSION, value => isPersistedOperationState(value) || isOperationRecordMap(value));
    if (state) {
...
export const createOperationState = options => {
  ...
  return {
    ...
    hydrate: () => {
      ...
      if (rawOps) {
        const state = decodeSupportedPersistence(rawOps, PERSISTENCE_SCHEMA_VERSION, value => isPersistedOperationState(value) || isOperationRecordMap(value));
        if (state) {
          const records = isPersistedOperationState(state) ? state.operations : state;
