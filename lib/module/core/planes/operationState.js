...
import { decodeSupportedPersistence, encodePersistence, jsonRoundTrip, PERSISTENCE_SCHEMA_VERSION } from "../persistenceCodec.js";
...
const isOperationRecord = value => isNonArrayRecord(value) && isNonEmptyString(value.operationId) && isNonEmptyString(value.model) && Array.isArray(value.tempIds) && value.tempIds.every(isNonEmptyString) && (value.rowIds === undefined || Array.isArray(value.rowIds) && value.rowIds.every(isNonEmptyString)) && (value.intent === 'insert' || value.intent === 'patch' || value.intent === 'destroy') && (value.status === 'pending' || value.status === 'committed' || value.status === 'rolledback' || value.status === 'failed') && isNonNegativeSafeInteger(value.createdAt) && (value.kind === undefined || isNonEmptyString(value.kind)) && (value.idempotencyKey === undefined || isNonEmptyString(value.idempotencyKey)) && (value.once === undefined || typeof value.once === 'boolean') && (value.patchedFields === undefined || Array.isArray(value.patchedFields) && value.patchedFields.every(isNonEmptyString)) && (value.patchedValues === undefined || isNonArrayRecord(value.patchedValues));
const isOperationRecordMap = value => isNonArrayRecord(value) && Object.entries(value).every(([operationId, record]) => isNonEmptyString(operationId) && isOperationRecord(record) && record.operationId === operationId);
const isOnceKeyRecord = value => isNonArrayRecord(value) && Array.isArray(value.keys) && value.keys.every(isNonEmptyString);
...
export const createOperationState = options => {
  ...
  const entriesFor = (records, onceKeys) => {
    ...
    return [{
      key: opsKey(),
      value: records.size > 0 ? encodePersistence(persistedRecords) : null
    }, {
      key: onceKeysKey(prefix()),
