...
import { noteCorruptionLedgerReset, noteDataLoss, noteUnknownOperationAck } from "../diagnostics.js";
...
const salvageLedgerValue = raw => {
  ...
  if (versioned && !staleVersion) {
    ...
    });else for (const entry of committedList) if (!isNonEmptyString(entry)) quarantined.push({
      id: 'ops-once',
      raw: entry,
      reason: 'corrupt-once-keys'
    });
