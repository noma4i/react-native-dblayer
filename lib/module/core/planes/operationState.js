...
export const readCommittedOnceKeys = (storage, prefix) => {
  ...
  if (rawOnceKeys) {
    try {
      const record = JSON.parse(rawOnceKeys);
  ...
  if (rawOperations) {
    try {
      for (const record of Object.values(JSON.parse(rawOperations))) {
...
export const writeCommittedOnceKeys = (storage, prefix, keys) => {
  ...
  storage.set([{
    ...
    value: JSON.stringify({
      formatVersion: ONCE_KEY_RECORD_FORMAT_VERSION,
      keys
    })
  }]);
};
...
export const createOperationState = options => {
  ...
  const persistEntries = () => {
    ...
    return [{
      ...
      key: onceKeysKey(prefix()),
      value: keys.length > 0 ? JSON.stringify({
        formatVersion: ONCE_KEY_RECORD_FORMAT_VERSION,
        keys
      }) : null
    }];
