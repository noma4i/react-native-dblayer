...
import { decodeVersionedRecord, encodePersistence, PERSISTENCE_SCHEMA_VERSION } from "./persistenceCodec.js";
...
export const registerCurrentManifestEntryProvider = provider => {
...
const isStorageResetIntent = value => isNonArrayRecord(value) && Number.isSafeInteger(value.recordVersion) && Array.isArray(value.restore) && value.restore.every(entry => isNonArrayRecord(entry) && typeof entry.key === 'string' && typeof entry.value === 'string');
...
export const resetRuntimeKeeping = restore => resetRuntimeWithRecovery(restore);
