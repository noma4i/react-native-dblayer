...
import { afterStoreTransaction, isInStoreTransaction, OWNED_COLLECTION_LIFETIME, SyncFeed, assertStoreReadable } from "./storeSync.js";
  ...
  return {
    scopeCollection: scopeKey => ({
      ...
      subscribe: listener => {
        ...
        let scheduled = false;
        let pending = [];
        const subscription = held.collection.subscribeChanges(changes => {
          const next = changes;
