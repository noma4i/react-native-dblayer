  ...
  const destroyEffects = (modelId, id, row) => {
    ...
    for (const relation of Object.values(host.relations())) {
      ...
      if (relation.kind === 'hasMany' && relation.dependent === 'destroy') {
        const ids = uniq(reader.rows(relation.model.modelId).filter(child => child[relation.foreignKey] === id).map(child => String(child.id))).filter(childId => !destroyed.has(compositeKey(relation.model.modelId, childId)));
