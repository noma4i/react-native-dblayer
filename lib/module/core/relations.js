...
const relationCollectionForQuery = collection => collection;
...
const useRowsByForeignKey = (relation, parentId) => {
  ...
  const {
    ...
    items: relationCollectionForQuery(relation.model.collection)
  }).where(({
    items
...
const useChildRowById = (context, childId) => {
  const {
    ...
    items: relationCollectionForQuery(context.collection)
  }).where(({
    items
...
const createHasOneAccessor = relation => ({
  ...
  use(parentId) {
    ...
    return useMemo(() => pickFirstRelatedRow(rows, relation.comparator), [rows, relation]);
  }
});
...
export const propagateBelongsToParents = (relations, row) => {
  if (!row) return;
  ...
  for (const relation of propagateRelations) {
    ...
    const patch = pickDefined(projected, Object.keys(projected));
