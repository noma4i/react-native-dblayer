...
import { noteNonResidentTouchDrop, noteRelationChildScan } from "./diagnostics.js";
import { pickLowestRow } from "./ordering.js";
 ...
 */
const hosts = createGenerationRegistry();
const facadeTargets = new Map();
export const registerRelationTarget = (key, target) => {
 ...
 */
export const readModelRelation = (modelId, id, name) => {
  ...
  if (relation.kind === 'hasOne') {
    ...
    return pickLowestRow(rows, relation.comparator);
  }
 ...
 */
export const deriveEffects = (accepted, destroyedRows, rawOps, reader) => {
  const queue = [];
  const out = [];
  ...
  const countKeyOf = (modelId, childId, counter) => compositeKey(modelId, childId, counter.model, counter.field);
  const replacedIdsByModel = new Map();
  for (const destroyedRow of destroyedRows) {
    if (destroyedRow.origin !== 'replace') continue;
    const ids = replacedIdsByModel.get(destroyedRow.model) ?? new Set();
    ids.add(destroyedRow.id);
    replacedIdsByModel.set(destroyedRow.model, ids);
  }
  const EMPTY_REPLACED_IDS = new Set();
  const accumulateTouch = (relation, childModelId, child, parentId) => {
    ...
    const patch = relation.touch(child, entry.view, {
      replacedIds: replacedIdsByModel.get(childModelId) ?? EMPTY_REPLACED_IDS
    });
    if (patch) {
   ...
   * | replace | none | yes | none |
   * | patch | none | none | none |
   * | ordinary destroy | -1 | none | cascade |
   * | replace destroy | none | none | none |
   *
   * Counters and cascades model logical existence: snapshots are authoritative and identity swaps
   * do not create or remove a record. Touch models observation of the newest child state: the
   * replace upsert leg carries the server identity fact, and the parent must see the server row in
   * the same envelope or its denormalized reference dangles on the destroyed temp id.
   */
  ...
  const destroyEffects = (modelId, id, row) => {
    ...
    for (const relation of Object.values(host.relations())) {
      ...
      if (relation.kind === 'hasMany' && relation.dependent === 'destroy') {
        const ids = uniq(childrenByForeignKey(relation.model.modelId, relation.foreignKey).get(id) ?? []).filter(childId => !destroyed.has(compositeKey(relation.model.modelId, childId)));
  ...
  for (const acceptedRow of accepted) {
    ...
    } else if (acceptedRow.origin === 'replace') {
      for (const relation of Object.values(host.relations())) {
        if (relation.kind !== 'belongsTo') continue;
  ...
  for (const op of rawOps) {
    ...
    const ids = placementIds.get(compositeKey(op.model, op.scopeKey)) ?? new Set();
