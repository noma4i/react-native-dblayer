...
import { decodePersistence, decodeVersionedRecord, encodePersistence, PERSISTENCE_SCHEMA_VERSION, versionPersistenceValue } from "../persistenceCodec.js";
...
export const readJournalRecord = (storage, prefix, journalKey) => {
  ...
  const decoded = decodeVersionedRecord(raw, PERSISTENCE_SCHEMA_VERSION, JOURNAL_RECORD_VERSION, value => isPersistedJournalRecord(value, journalEpoch));
  if (decoded.kind === 'unsupported') throw new Error(`Unsupported persistence schema version ${decoded.schemaVersion}`);
