...
import { noteCorruptionJournalDrop, noteCorruptionJournalLoss } from "../diagnostics.js";
...
export const createJournal = (storage, prefix) => {
  ...
  const allRecords = () => storage.keys(key('journal:')).map(journalKey => readJournalRecord(storage, prefix(), journalKey)).filter(record => record !== null).sort((a, b) => a.epoch - b.epoch);
