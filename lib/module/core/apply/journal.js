...
const hasJournalIdentity = value => isNonEmptyString(value.txId) && isPositiveSafeInteger(value.runtimeEpoch) && isPositiveSafeInteger(value.epoch);
const hasVersionedValues = value => Array.isArray(value) && value.every(entry => isNonArrayRecord(entry) && isPositiveSafeInteger(entry.schemaVersion) && 'payload' in entry);
