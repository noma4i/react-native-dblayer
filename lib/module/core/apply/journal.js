...
import { decodePersistence, decodeSupportedPersistence, encodePersistence, PERSISTENCE_SCHEMA_VERSION, versionPersistenceValue } from "../persistenceCodec.js";
...
const checkpointEpoch = (storage, prefix) => {
  ...
  if (!raw) return 0;
  const parsed = decodeSupportedPersistence(raw, PERSISTENCE_SCHEMA_VERSION, value => typeof value === 'object' && value !== null && typeof value.lastCheckpointEpoch === 'number');
...
const isValidJournalOp = value => {
  ...
  if (op.kind === 'upsert') return Array.isArray(op.rows) && op.rows.every(row => typeof row === 'object' && row !== null && typeof row.id === 'string');
  ...
  if (op.kind === 'scope-delta') return typeof op.scopeKey === 'string' && Array.isArray(op.append) && op.append.every(row => typeof row === 'object' && row !== null && typeof row.id === 'string') && Array.isArray(op.detach) && op.detach.every(id => typeof id === 'string');
  return false;
};
const isPersistedJournalRecord = value => {
  if (typeof value !== 'object' || value === null) return false;
  const record = value;
  return typeof record.txId === 'string' && typeof record.runtimeEpoch === 'number' && typeof record.epoch === 'number' && (record.status === 'pending' || record.status === 'committed') && Array.isArray(record.ops) && record.ops.every(op => typeof op === 'object' && op !== null && typeof op.schemaVersion === 'number' && 'payload' in op);
};
...
export const createJournal = (storage, prefix) => {
  ...
  const allRecords = () => sortBy(storage.keys(key('journal:')).map(journalKey => readJournalRecord(storage, prefix(), journalKey)).filter(record => record !== null), [record => record.epoch]);
