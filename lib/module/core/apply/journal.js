...
const isValidJournalOp = value => {
  ...
  const op = value;
  if (op.kind === 'upsert') return Array.isArray(op.rows) && op.rows.every(row => isNonArrayRecord(row) && isNonEmptyString(row.id)) && (op.origin === undefined || op.origin === 'replace');
  if (op.kind === 'destroy') return Array.isArray(op.ids) && op.ids.every(isNonEmptyString) && (op.tombstone === undefined || typeof op.tombstone === 'boolean') && (op.origin === undefined || op.origin === 'replace');
...
const isPersistedJournalRecord = value => {
  ...
  const record = value;
  return isNonEmptyString(record.txId) && isPositiveSafeInteger(record.runtimeEpoch) && isPositiveSafeInteger(record.epoch) && (record.status === 'pending' || record.status === 'committed') && Array.isArray(record.ops) && record.ops.every(op => isNonArrayRecord(op) && isPositiveSafeInteger(op.schemaVersion) && 'payload' in op);
};
...
export const createJournal = (storage, prefix) => {
  ...
  const planPrune = (index, virtual, pruneBeforeEpoch) => {
  ...
  const pruneCommitted = pruneBeforeEpoch => {
    ...
    const {
      stale,
      commitStale
    } = planPrune(index, index, pruneBeforeEpoch);
    return {
      entries: stale.map(epoch => ({
        key: recordKey(epoch),
        value: null
      })),
      commit: commitStale
  ...
  return {
    ...
    committedEntry: (record, pruneBeforeEpoch = Number.POSITIVE_INFINITY) => {
      ...
      const {
        stale,
        commitStale
      } = planPrune(index, virtual, pruneBeforeEpoch);
      const entries = [{
        key: recordKey(record.epoch),
