...
import { encodePersistence } from "../persistenceCodec.js";
...
export const createCheckpointScheduler = options => {
  ...
  function flushNow() {
    ...
    if (entries.length === 0 && markers.length === 0 && extraEntries.length === 0) {
      plans = 0;
      return;
    }
    ...
    for (const entry of entries) options.storage.set(entry.key, entry.value);
  ...
  const checkpointDeadline = new Debouncer(flushNow, {
