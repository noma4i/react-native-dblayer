...
import { isWhereOperatorValue } from "./compileDbWhere.js";
const isOperatorNode = where => isNonArrayRecord(where) && ('and' in where || 'or' in where || 'not' in where);

/** A condition that admits every row. The compiler is total, so a caller never branches on "no filter". */
const MATCHES_EVERY_ROW = () => eq(1, 1);
const allOf = parts => {
 ...
 */
const missing = field => or(isNull(field), isUndefined(field));
const comparisons = {
 ...
 */
const equality = (field, value) => value === null ? isNull(field) : eq(field, value);
const leafExpression = (ref, condition) => allOf(Object.entries(condition).filter(([, value]) => value !== undefined).flatMap(([key, value]) => isWhereOperatorValue(value) ? operatorExpressions(ref[key], value) : [equality(ref[key], value)]));
 ...
 */
export const compileWhereExpression = (ref, where) => {
  ...
  return not(coalesce(compileWhereExpression(ref, where.not), false));
};
