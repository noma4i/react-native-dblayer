...
import { planOptimisticInsert } from "../dsl/optimisticInsertPlanners.js";
import { createCommitEnvelope } from "./apply/commitEnvelope.js";
import { getApplyTarget } from "./apply/applyTargetRegistry.js";
import { getInternalModelHandleById } from "./internalHandles.js";
import { putQuarantine, readQuarantineEntries, takeQuarantineEntries } from "./quarantine.js";
...
const closeCrashedRequests = () => {
  ...
  for (const operation of crashedRequests) {
    ...
    const planned = planRequestFailureRollback(operation, id => hasApplyTarget(operation.model) ? getApplyTarget(operation.model).readRow(id) : undefined,
