...
const isQueryInvalidationRecord = value => isRecord(value) && value.recordVersion === QUERY_INVALIDATION_RECORD_VERSION && typeof value.revision === 'number' && Number.isSafeInteger(value.revision) && value.revision >= 0 && isRecord(value.identities) && Object.values(value.identities).every(revision => typeof revision === 'number' && Number.isSafeInteger(revision) && revision >= 0);
