...
export const isOrderKey = value => typeof value === 'string' && /^[0-9A-Za-z]*[1-9A-Za-z]$/.test(value);
 ...
 */
export const keyBetween = (lower, upper) => {
  let prefix = '';
...
export const keyBefore = key => keyBetween(undefined, key);

/** Generate a key strictly after `key` (an append placement). */
export const keyAfter = key => keyBetween(key, undefined);
 ...
 */
export const keysForSequence = (count, lower, upper) => {
  if (count <= 0) return [];
