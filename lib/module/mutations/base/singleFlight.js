...
export const serializeSingleFlightValue = value => {
  if (value === undefined) {
    return 'undefined';
  }
  return JSON.stringify(normalizeForStableSerialization(value));
};
export const createSingleFlightSignature = (scope, mutationKey, payload) => {
  return `${scope}:${serializeSingleFlightValue(mutationKey)}:${serializeSingleFlightValue(payload)}`;
};
export const runSingleFlight = (signature, execute) => {
