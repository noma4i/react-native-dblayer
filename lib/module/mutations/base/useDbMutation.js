...
import { applyDbMutationCommit, executeDbMutationLifecycle, readMutationTempId } from "./executeDbMutation.js";
...
const runDeclarativeOptimisticMutation = (optimistic, input) => {
  const existingTempId = optimistic.selectTempId ? optimistic.selectTempId(input) : readMutationTempId(input);
  if (existingTempId) {
    return {
      ...
      return executeDbMutationLifecycle(config, input, mappedInput, {
        ...
        beforeRequest: () => {
          ...
          if (shouldRunOptimisticMutation(config)) {
            tx.mutate(() => {
              context = runInManagedMutationBatch(() => runOptimisticMutation(config, input));
            });
          }
          return context;
        },
