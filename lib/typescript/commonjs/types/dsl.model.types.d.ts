...
export type ModelWrites<TStored extends {
    ...
    prepareRow(row: unknown, previous: TStored | undefined, origin?: Exclude<WriteOrigin, 'patch' | 'snapshot'>, mergeBase?: TStored, operationId?: string, baseRevision?: number): import('./core.apply.transaction.types').PreparedRowWrite | null;
    preparePatch(id: string, patch: Record<string, unknown>, previous: TStored | undefined, operationId?: string, remove?: readonly string[], baseRevision?: number): import('./core.apply.transaction.types').PreparedRowWrite | null;
...
export type ModelSchemaRegistrationOptions<TStored extends {
    id: string;
} & Record<string, unknown>> = {
    modelId: string;
    modelName: string;
...
export type ModelDirectAccess<TStored extends {
    id: string;
    updatedAt?: string | null;
}, TInput> = Pick<ModelCore<TStored, TInput>, 'find' | 'where' | 'all' | 'update' | 'destroy' | 'destroyMany' | 'updateAll' | 'destroyAll' | 'insert' | 'insertMany' | 'seed' | 'replace' | 'build' | 'normalize' | 'invalidate'>;
export type ModelDirectAccessOptions<TStored extends {
    id: string;
    updatedAt?: string | null;
} & Record<string, unknown>, _TInput> = {
    modelId: string;
...
export type RequiredReadUse<TStored extends {
    id: string;
    updatedAt?: string | null;
...
export type QueryScopeSpec<TStored extends {
    id: string;
}> = {
...
//# sourceMappingURL=dsl.model.types.d.ts.map