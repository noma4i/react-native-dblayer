export type DataLossMechanism = 'deferred-patch-timeout' | 'gc-row-eviction' | 'gc-scope-removal' | 'gc-scope-membership-detach' | 'scope-retention-trim' | 'scope-complete-detach' | 'stale-temp-row-expiry' | 'tombstone-expiry' | 'corrupt-row' | 'corrupt-tombstones' | 'corrupt-scope' | 'corrupt-applied-epoch' | 'corrupt-checkpoint-meta' | 'failed-input-unserializable' | 'detached-operation-discard' | 'detached-operation-rollback' | 'detached-resume-error' | 'data-version-migration-reset' | 'model-corruption-recovery' | 'journal-corruption-checkpointed-drop' | 'journal-corruption-loss' | 'operation-ledger-corruption-reset' | 'corrupt-once-keys' | 'replacement-rejected' | 'unranked-ladder-value';
