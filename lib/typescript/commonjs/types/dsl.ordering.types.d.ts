...
export type OrderableField<TStored> = Extract<IsAny<TStored> extends true ? string : string extends keyof TStored ? string : {
...
export type ReadOrder<TStored> = {
    field: OrderableField<TStored>;
    direction: 'asc' | 'desc';
};
/** Field-based scope order over one orderable stored field. */
export type FieldSort<TStored> = IsAny<TStored> extends true ? {
    field: string;
    dir: 'asc' | 'desc';
    ...
    [K in keyof TStored]-?: IsAny<TStored[K]> extends true ? never : TStored[K] extends OrderableValue ? {
...
export type ComparatorSort<TStored> = {
    comparator: {
        call(a: TStored, b: TStored): number;
    }['call'];
