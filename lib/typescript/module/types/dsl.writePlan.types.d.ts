...
type ForeignWriteTarget<TOwnerKey extends string, TTarget> = TTarget extends {
    readonly key: infer TKey;
} ? string extends TOwnerKey ? TTarget : TKey extends TOwnerKey ? never : TTarget : TTarget;
export type WritePlan<TOwnerKey extends string = never> = {
    /** Declares rows to insert or replace in a model. */
    upsert<TTarget extends AnyWriteTarget>(model: ForeignWriteTarget<TOwnerKey, TTarget>, rowOrRows: WriteTargetInput<TTarget> | readonly WriteTargetInput<TTarget>[]): void;
    /** Declares a partial update for one stored row. */
    update<TTarget extends AnyWriteTarget>(model: ForeignWriteTarget<TOwnerKey, TTarget>, id: string, patch: Partial<WriteTargetStored<TTarget>>): void;
    /** Declares destruction of one or more stored rows. */
    destroy<TTarget extends AnyWriteTarget>(model: ForeignWriteTarget<TOwnerKey, TTarget>, idOrIds: string | readonly string[]): void;
