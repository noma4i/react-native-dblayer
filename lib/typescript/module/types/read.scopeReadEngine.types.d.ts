...
export type ScopeProjectionOptions<TOutput extends Record<string, unknown>> = ProjectionOptions<RowRecord, TOutput> & {
    keepPrevious?: boolean;
    require?: ReadonlyArray<string>;
};
/** One window snapshot handed to scope window readers, with retention and resolution flags. */
export type ScopeWindowSnapshot = {
    rows: RowRecord[];
    totalCount: number;
...
export type RequireGate = {
    source: RowRecord[] | null;
    require: ReadonlyArray<string> | undefined;
    result: RowRecord[];
};
/** Work counters for one scope read pass, split into full and incremental row costs. */
export type ScopeReadWorkSnapshot = {
    fullRows: number;
    incrementalRows: number;
};
//# sourceMappingURL=read.scopeReadEngine.types.d.ts.map