...
var _optimisticInsertPlanners = require("../dsl/optimisticInsertPlanners.js");
...
const closeCrashedRequests = () => {
  ...
  for (const operation of crashedRequests) {
    ...
    if (tempId !== null && Object.hasOwn(operation, 'input') && hasApplyTarget(operation.model) && (0, _applyTargetRegistry.getApplyTarget)(operation.model).readRow(tempId) === undefined) {
      recoveryOps.push(...((0, _optimisticInsertPlanners.planOptimisticInsert)(operation.model, operation.actionKey, operation.input, tempId, operation.operationId) ?? []));
    }
