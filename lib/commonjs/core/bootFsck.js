...
const closeCrashedRequests = () => {
  ...
  for (const operation of crashedRequests) {
    const planned = (0, _requestRollback.planRequestFailureRollback)(operation, id => hasApplyTarget(operation.model) ? (0, _applyTargetRegistry.getApplyTarget)(operation.model).readRow(id) : undefined, (row, memberships) => [{
      kind: 'upsert',
