...
exports.waitForRow = exports.patchWhenPresent = exports.getRowWaiterDebugInfo = exports.clearRowWaitersForCollection = void 0;
...
const expirePatchQueue = (state, queue) => {
  const current = finishPatchQueue(state, queue);
  if (!current) return;
  (0, _logger.getDbLogger)().debug('db', 'row patch queue expired', {
...
const clearRowWaitersForCollection = collection => {
...
exports.clearRowWaitersForCollection = clearRowWaitersForCollection;
const getRowWaiterDebugInfo = collection => {
  const state = states.get(collection);
  return {
...
exports.getRowWaiterDebugInfo = getRowWaiterDebugInfo;
