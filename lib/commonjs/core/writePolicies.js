...
const compareTupleValue = (incoming, current) => {
  ...
  const incomingNumber = (0, _normalizeHelpers.readNumericLike)(incoming);
  const currentNumber = (0, _normalizeHelpers.readNumericLike)(current);
...
const isIncomingNewerBy = (path, incoming, previous) => (0, _invariants.isIncomingNewer)((0, _normalizeHelpers.readIsoDate)(readPath(previous, path)), (0, _normalizeHelpers.readIsoDate)(readPath(incoming, path)));
