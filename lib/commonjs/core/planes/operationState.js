...
const isOperationRecord = value =>
  (0, _normalizeHelpers.isNonArrayRecord)(value) &&
  (0, _normalizeHelpers.isNonEmptyString)(value.operationId) &&
  (0, _normalizeHelpers.isNonEmptyString)(value.model) &&
  Array.isArray(value.tempIds) &&
  value.tempIds.every(_normalizeHelpers.isNonEmptyString) &&
  (value.rowIds === undefined || (Array.isArray(value.rowIds) && value.rowIds.every(_normalizeHelpers.isNonEmptyString))) &&
  ...
  (0, _normalizeHelpers.isNonNegativeSafeInteger)(value.createdAt) &&
  (value.kind === undefined || (0, _normalizeHelpers.isNonEmptyString)(value.kind)) &&
  (value.idempotencyKey === undefined || (0, _normalizeHelpers.isNonEmptyString)(value.idempotencyKey)) &&
  (value.once === undefined || typeof value.once === 'boolean') &&
  (value.patchedFields === undefined || (Array.isArray(value.patchedFields) && value.patchedFields.every(_normalizeHelpers.isNonEmptyString))) &&
  (value.patchedValues === undefined || (0, _normalizeHelpers.isNonArrayRecord)(value.patchedValues));
const isOperationRecordMap = value =>
  (0, _normalizeHelpers.isNonArrayRecord)(value) &&
  Object.entries(value).every(([operationId, record]) => (0, _normalizeHelpers.isNonEmptyString)(operationId) && isOperationRecord(record) && record.operationId === operationId);
  ...
  const entriesFor = (records, onceKeys) => {
    ...
    return [
      ...
      },
      {
        ...
        value:
          keys.length > 0
            ? (0, _persistenceCodec.encodePersistence)({
                keys
              })
            : null
      }
  ...
  return {
    ...
    latestPendingValue: (model, rowId, field, excludeOpId) => {
      ...
      for (const operation of bucket) {
        ...
        if (operation.model !== model || !(operation.rowIds ?? []).includes(rowId)) continue;
        if (operation.patchedValues && field in operation.patchedValues)
          result = {
            found: true,
      ...
      if (rawOps) {
        ...
        if (records) {
          for (const [operationId, record] of Object.entries(records)) {
            ...
            const hydratedRecord = retainKey
              ? record
              : {
                  ...record,
