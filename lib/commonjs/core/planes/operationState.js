...
const isOperationRecord = value => (0, _normalizeHelpers.isRecord)(value) && typeof value.operationId === 'string' && typeof value.model === 'string' && Array.isArray(value.tempIds) && value.tempIds.every(id => typeof id === 'string') && (value.rowIds === undefined || Array.isArray(value.rowIds) && value.rowIds.every(id => typeof id === 'string')) && (value.intent === 'insert' || value.intent === 'patch' || value.intent === 'destroy') && (value.status === 'pending' || value.status === 'committed' || value.status === 'rolledback' || value.status === 'failed') && typeof value.createdAt === 'number';
const isOperationRecordMap = value => (0, _normalizeHelpers.isRecord)(value) && Object.entries(value).every(([operationId, record]) => isOperationRecord(record) && record.operationId === operationId);
const isOnceKeyRecord = value => (0, _normalizeHelpers.isRecord)(value) && Array.isArray(value.keys) && value.keys.every(key => typeof key === 'string');
...
const createOperationState = options => {
  ...
  const persistEntries = () => {
    ...
    return [{
      key: opsKey(),
      value: operations.size > 0 ? (0, _persistenceCodec.encodePersistence)(Object.fromEntries(operations)) : null
    }, {
      key: onceKeysKey(prefix()),
