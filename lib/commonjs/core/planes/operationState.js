...
exports.writeCommittedOnceKeys = exports.serializeOperationInput = exports.readCommittedOnceKeys = exports.isOperationTransition = exports.isOperationRecord = exports.createOperationState = exports.committedOnceKeysEntry = void 0;
...
const isPersistedOperationState = value => (0, _normalizeHelpers.isNonArrayRecord)(value) && value.recordVersion === OPERATION_STATE_RECORD_VERSION && isOperationRecordMap(value.operations) && Array.isArray(value.committedKeys) && value.committedKeys.every(_normalizeHelpers.isNonEmptyString);
...
const readCommittedOnceKeys = (storage, prefix) => {
  ...
  if (rawOperations) {
    const state = (0, _persistenceCodec.decodeSupportedPersistence)(rawOperations, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, value => isPersistedOperationState(value) || isOperationRecordMap(value));
    if (state) {
...
const createOperationState = options => {
  ...
  return {
    ...
    hydrate: () => {
      ...
      if (rawOps) {
        const state = (0, _persistenceCodec.decodeSupportedPersistence)(rawOps, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, value => isPersistedOperationState(value) || isOperationRecordMap(value));
        if (state) {
          const records = isPersistedOperationState(state) ? state.operations : state;
