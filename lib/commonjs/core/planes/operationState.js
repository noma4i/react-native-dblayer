...
const salvageLedgerValue = raw => {
  ...
  const candidates = versioned ? (0, _normalizeHelpers.isNonArrayRecord)(value.operations) ? value.operations : {} : value;
  const operations = {};
  const quarantined = [];
  ...
  if (versioned && !staleVersion) {
    ...
    if (committedList === undefined) quarantined.push({
      id: 'ops-once',
      raw: value.committedKeys ?? null,
      reason: 'corrupt-once-keys'
    });else for (const entry of committedList) if (!(0, _normalizeHelpers.isNonEmptyString)(entry)) quarantined.push({
      id: 'ops-once',
      raw: entry,
      reason: 'corrupt-once-keys'
    });
  }
  const committedKeys = versioned && committedList ? committedList.filter(_normalizeHelpers.isNonEmptyString) : [];
  return {
    kind: 'salvaged',
    operations,
    committedKeys,
  ...
  return {
    ...
    close: (operationId, status) => {
      ...
      if (!operation || operation.status !== 'pending') {
        (0, _diagnostics.noteUnknownOperationAck)();
        return;
      }
    ...
    hydrate: () => {
      ...
      if (rawOps) {
        ...
        if (salvage.kind === 'unreadable') {
          ...
          (0, _logger.getDbLogger)().error('ledger quarantined', {
            key: opsKey()
          });
        } else {
          ...
          for (const entry of salvage.quarantined) {
            const model = (0, _normalizeHelpers.isNonArrayRecord)(entry.raw) && (0, _normalizeHelpers.isNonEmptyString)(entry.raw.model) ? entry.raw.model : '__operations__';
            (0, _quarantine.putQuarantine)({
              kind: 'operation',
              model,
              id: entry.id,
          ...
          (0, _diagnostics.noteDataLoss)('corrupt-once-keys', '__operations__', salvage.quarantined.filter(entry => entry.reason === 'corrupt-once-keys').length);
