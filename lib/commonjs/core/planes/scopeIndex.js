...
const isScopeEntry = value =>
  (0, _normalizeHelpers.isNonArrayRecord)(value) &&
  (0, _normalizeHelpers.isNonEmptyString)(value.id) &&
  (0, _orderKey.isOrderKey)(value.orderKey) &&
  (value.edge === undefined || (0, _normalizeHelpers.isNonArrayRecord)(value.edge));
...
const isScopeIndexValue = value => {
  ...
  if (
    !(0, _normalizeHelpers.isNonNegativeSafeInteger)(value.generation) ||
    (value.coverage !== 'delta' && value.coverage !== 'page' && value.coverage !== 'complete') ||
    !isScopeEntrySet(entries)
  )
    return false;
...
const createScopeIndex = options => {
  ...
  const trimValue = (value, maxRows) => {
    if (value.entries.length <= maxRows)
      return {
        next: value,
        trimmedIds: []
      };
  ...
  return {
    ...
    applyDelta: (key, append, detach) => {
      ...
      const pureAppend =
        ...
        (base.length === 0 || append.every(entry => (0, _serialize.compareCodepoints)(entry.orderKey, base.at(-1).orderKey) > 0));
    ...
    hydrate: () => {
      ...
      for (const fullKey of storage.keys(storagePrefix())) {
        ...
        const declared =
          scopeParts?.length === 2 &&
          (0, _normalizeHelpers.isNonEmptyString)(scopeParts[0]) &&
          (0, _normalizeHelpers.isNonEmptyString)(scopeParts[1]) &&
          (scopeNames === undefined || scopeNames.includes(scopeParts[0]));
