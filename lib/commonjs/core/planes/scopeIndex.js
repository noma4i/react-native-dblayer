  ...
  const reconcileNext = (key, coverage, incoming, opts) => {
    ...
    if (coverage === 'complete') {
      ...
      const entries = deduplicated.map((row, index) => {
        ...
        let key = keys[index];
        while (taken.has(key)) key = (0, _orderKey.keysForSequence)(1, key, keys[index + 1])[0];
        taken.add(key);
  ...
  return {
    ...
    persistEntries: () => {
      ...
      for (const key of changed) {
        ...
        entries.push({
          key: storageKey(key),
          value: value ? (0, _persistenceCodec.encodePersistence)(value) : null
        });
      }
      return entries;
    },
    ...
    hydrate: () => {
      ...
      for (const fullKey of storage.keys(storagePrefix())) {
        ...
        const value = raw && declared ? (0, _persistenceCodec.decodeSupportedPersistence)(raw, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, isScopeIndexValue) : undefined;
