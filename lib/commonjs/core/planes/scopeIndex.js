...
const isScopeIndexValue = value => (0, _normalizeHelpers.isRecord)(value) && typeof value.generation === 'number' && (value.coverage === 'delta' || value.coverage === 'page' || value.coverage === 'complete') && Array.isArray(value.entries) && value.entries.every(entry => (0, _normalizeHelpers.isRecord)(entry) && typeof entry.id === 'string' && typeof entry.order === 'number' && typeof entry.seq === 'number' && (entry.edge === undefined || (0, _normalizeHelpers.isRecord)(entry.edge)));
const createScopeIndex = options => {
  ...
  const reconcileNext = (key, coverage, incoming, opts) => {
    ...
    if (boundaryAdd) {
      const sortedIncoming = (0, _esToolkit.sortBy)(incoming, [row => row.order, row => row.id]).map(row => ({
        id: row.id,
        order: row.order,
        seq: generation,
        edge: row.edge
  ...
  return {
    ...
    reconcile: (key, coverage, incoming, opts) => {
      ...
      if (result.detachedIds.length > 0) (0, _diagnostics.noteDataLoss)('scope-complete-detach', modelId, result.detachedIds.length);
      return {
    ...
    trim: (key, maxRows) => {
      ...
      if (result.trimmedIds.length > 0) {
        ...
        (0, _diagnostics.noteDataLoss)('scope-retention-trim', modelId, result.trimmedIds.length);
      }
    ...
    hydrate: () => {
      ...
      for (const fullKey of storage.keys(storageKey(''))) {
        ...
        const canonicalScopeName = orderedScopeNames.find(scopeName => persistedKey.startsWith((0, _serialize.compositeKey)(scopeName, '')));
        ...
        const key = canonicalScopeName ? persistedKey : colonDelimitedScopeName ? (0, _serialize.compositeKey)(colonDelimitedScopeName, persistedKey.slice(colonDelimitedScopeName.length + 1)) : null;
        if (!key) {
          storage.set([{
            key: fullKey,
            value: null
        ...
        if (!raw) continue;
        const value = (0, _persistenceCodec.decodeSupportedPersistence)(raw, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, isScopeIndexValue);
        if (value) {
          const entry = {
            raw,
            value,
      ...
      if (migrated.size > 0) {
        ...
        storage.set(entries);
        (0, _diagnostics.noteScopeKeyMigration)(migrated.size);
      }
      memberSets.clear();
      keysByRow.clear();
