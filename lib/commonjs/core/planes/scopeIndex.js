...
const isScopeEntry = value => (0, _normalizeHelpers.isNonArrayRecord)(value) && (0, _normalizeHelpers.isNonEmptyString)(value.id) && (0, _orderKey.isOrderKey)(value.orderKey) && (value.edge === undefined || (0, _normalizeHelpers.isNonArrayRecord)(value.edge));
...
const createScopeIndex = options => {
  ...
  const reconcileNext = (key, coverage, incoming, opts) => {
    ...
    if (coverage === 'complete') {
      ...
      if ((0, _arrayEquality.arraysShallowEqual)(previous.entries, deduplicated.map(row => row.id), (entry, id) => entry.id === id)) {
