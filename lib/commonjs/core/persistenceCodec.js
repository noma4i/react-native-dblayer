...
exports.versionPersistenceValue = exports.jsonRoundTrip = exports.encodePersistence = exports.decodeVersionedRecord = exports.decodeSupportedPersistence = exports.decodePersistence = exports.PERSISTENCE_SCHEMA_VERSION = void 0;
...
const isRecordWithVersion = value => typeof value === 'object' && value !== null && !Array.isArray(value) && Number.isSafeInteger(value.recordVersion);
 ...
 */
const decodeVersionedRecord = (raw, expectedSchemaVersion, expectedRecordVersion, accepts) => {
  const decoded = decodePersistence(raw, expectedSchemaVersion, isRecordWithVersion);
  if (decoded.kind !== 'ok') return decoded;
  if (decoded.value.recordVersion !== expectedRecordVersion) return {
    kind: 'stale-version',
    recordVersion: decoded.value.recordVersion
  };
  return accepts(decoded.value) ? {
    kind: 'ok',
    value: decoded.value
  } : {
    kind: 'corrupt'
...
exports.decodeVersionedRecord = decodeVersionedRecord;
