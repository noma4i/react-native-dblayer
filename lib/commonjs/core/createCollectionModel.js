...
var _rowVersionCore = require("./rowVersionCore.js");
...
const hasCompleteFields = (fields, normalized) => {
...
function createCollectionModel(config) {
  ...
  const writePropagation = (0, _writePropagation.createWritePropagation)(config.name);
  ...
  const versionCore = (0, _rowVersionCore.createRowVersionCore)();
  let resetMergeState = () => {};
    ...
    (0, _sideload.withApplyingModel)(config.name, () => {
      withTransaction(() => {
        ...
        if (internalContract.mode === 'replace') {
          ...
          result = replace(items, scopeFilter, internalContract.snapshotSeq);
        } else {
          result = merge(items, internalContract.snapshotSeq);
        }
      });
    });
  ...
  const baseModel = {
    ...
    getCollectionWriteSeq: versionCore.currentSeq,
    getRowWriteSeq: versionCore.getWriteSeq,
    getRowDeleteSeq: versionCore.getDeleteSeq,
    applyServerData,
    markFetched: freshness.markFetched,
    getFetchState: freshness.getFetchState,
    clearFetchState: freshness.clearFetchState,
    invalidate: scope => (0, _queryClient.invalidateModel)(modelBase, scope),
    shouldSkipInitialFetch,
    clearScope,
    find: useFind,
    all: useAll,
  ...
  const statics = config.statics?.(modelBase);
  if (!statics) {
