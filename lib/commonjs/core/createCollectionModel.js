...
const buildFetchScope = filter => {
  ...
  return {
    scope: (0, _serialize.stableSerialize)(normalized),
    filter: normalized
  };
};
const createFreshnessTracker = (modelName, collectionId, staleTime, emptyStaleTime) => {
  ...
  const markFetched = (filter, state) => {
    ...
    fetchStateCache.set(scope, nextState);
    if (collectionId) {
      (0, _freshnessStorage.setCollectionFetchState)(collectionId, nextState, scope === ROOT_FETCH_SCOPE ? undefined : scope, normalizedFilter);
    }
  };
  const touch = () => {
