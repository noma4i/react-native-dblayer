...
exports.resumeFetchReaders = exports.registerActiveFetchReaders = exports.refetchActiveFetchReaders = void 0;
...
const serializedKeyOf = queryKey => JSON.stringify(queryKey);
 ...
 */
const noteLostRows = batch => {
  if (!(0, _configure.isDbConfigured)()) return;
  const lost = new Set(batch.rows.filter(row => row.kind === 'destroy').map(row => (0, _serialize.compositeKey)(row.model, row.id)));
...
(0, _configure.getCommitBus)().subscribeAll(noteLostRows);
