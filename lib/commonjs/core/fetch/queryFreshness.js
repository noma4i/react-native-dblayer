...
exports.resolveStaleTime = exports.persistenceWindowOf = exports.isQueryFresh = void 0;
...
const persistenceWindowOf = (empty, staleTime, emptyStaleTime, defaults) => {
  const window = empty ? resolveStaleTime(emptyStaleTime, defaults) ?? defaults.emptyStaleTime ?? resolveStaleTime(staleTime, defaults) ?? defaults.staleTime ?? 0 : resolveStaleTime(staleTime, defaults) ?? defaults.staleTime ?? 0;
  return Number.isFinite(window) && window > 0 ? window : null;
};
exports.persistenceWindowOf = persistenceWindowOf;
