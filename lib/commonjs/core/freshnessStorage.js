...
exports.subscribeCollectionFetchState = exports.setCollectionFetchState = exports.registerCollectionFetchStateCache = exports.pruneStaleFetchStates = exports.listCollectionFetchScopes = exports.getCollectionFetchStateVersion = exports.getCollectionFetchState = exports.clearCollectionFetchStates = exports.clearCollectionFetchState = exports.clearAllFreshnessMetadata = exports.DEFAULT_FETCH_STATE_MAX_AGE_MS = void 0;
...
const buildFreshnessKey = (collectionId, scopeKey = _compileDbWhere.ROOT_SCOPE_KEY) => `${FRESHNESS_KEY_PREFIX}${collectionId}:${scopeKey}`;
...
const parseFreshnessKey = key => {
  ...
  return {
    collectionId,
    scopeKey: scopeKey === _compileDbWhere.ROOT_SCOPE_KEY ? undefined : scopeKey
  };
};
...
const removeFreshnessKey = (key, options) => {
  ...
  const existed = (0, _storage.getDbStorageAdapter)().getItem(key) !== null;
...
const parsePersistedFetchState = raw => {
  ...
  try {
    ...
    return {
      ...state,
      ...((0, _normalizeHelpers.isNonArrayRecord)(parsed._freshnessFilter) ? {
...
const parseScopeFromKey = scopeKey => {
  ...
  try {
    ...
    return (0, _normalizeHelpers.isNonArrayRecord)(parsed) ? parsed : undefined;
  } catch {
    return undefined;
  }
};
...
const setCollectionFetchState = (collectionId, state, scopeKey, scope) => {
  const persisted = {
    ...state,
    ...(scope ? {
  ...
  (0, _storage.getDbStorageAdapter)().setItem(buildFreshnessKey(collectionId, scopeKey), JSON.stringify(persisted));
...
const listCollectionFetchScopes = collectionId => {
  const prefix = buildCollectionPrefix(collectionId);
  const records = [];
  for (const key of (0, _storage.getDbStorageAdapter)().getAllKeys()) {
    ...
    const persisted = parsePersistedFetchState((0, _storage.getDbStorageAdapter)().getItem(key));
...
exports.listCollectionFetchScopes = listCollectionFetchScopes;
...
const subscribeCollectionFetchState = (collectionId, listener) => {
  const listeners = collectionFetchStateSubscribers.get(collectionId);
  if (listeners) {
    listeners.add(listener);
  } else {
...
exports.subscribeCollectionFetchState = subscribeCollectionFetchState;
const getCollectionFetchStateVersion = collectionId => collectionFetchStateVersions.get(collectionId) ?? 0;

/** Remove stale or invalid fetch-state metadata and return the number removed. */
exports.getCollectionFetchStateVersion = getCollectionFetchStateVersion;
