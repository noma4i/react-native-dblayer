...
exports.createCommitEnvelope = void 0;
...
const readPlannedRow = (overlay, model, id) => {
  ...
  return (0, _applyTargetRegistry.getApplyTarget)(model).readRow(id);
};
...
const readPlannedRows = (overlay, model) => {
  const rows = new Map((0, _applyTargetRegistry.getApplyTarget)(model).readAllRows().flatMap(row => typeof row.id === 'string' ? [[row.id, row]] : []));
  for (const [id, row] of overlay.get(model) ?? []) {
    if (row === null) rows.delete(id);else rows.set(id, row);
  }
  return [...rows.values()];
};
const prepareOperations = (ops, overlay, rejectedReplacements, rejectedReplaceSources) => {
  const preparedOps = [];
  const accepted = [];
  const destroyed = [];
  ...
  for (const op of ops) {
    ...
    if (op.kind === 'upsert') {
      ...
      for (const input of op.rows) {
        ...
        if (op.origin === 'replace' && inputId && !prepared) {
          rejectedReplacements.add((0, _serialize.compositeKey)(op.model, inputId));
          ...
          if (op.replaceOf !== undefined) {
            rejectedReplaceSources.add((0, _serialize.compositeKey)(op.model, op.replaceOf));
            (0, _quarantine.putQuarantine)({
              kind: 'row',
              model: op.model,
              id: inputId,
              raw: input,
              reason: 'replace-rejected-by-admission'
            });
          }
        }
    ...
    if (op.kind === 'destroy') {
      ...
      const destroyIds = op.ids.map(String).filter(id => op.origin !== 'replace' || !rejectedReplaceSources.has((0, _serialize.compositeKey)(op.model, id)));
    ...
    if (op.kind === 'scope-delta') {
      const append = (0, _scopeIndex.deduplicateScopeEntriesById)(op.append);
      ...
      preparedOps.push({
        ...
        detach: op.detach.filter(id => !rejectedReplaceSources.has((0, _serialize.compositeKey)(op.model, id)))
      });
      continue;
    }
    preparedOps.push(op);
...
const compileWritePlan = initialOps => {
  for (const op of initialOps) (0, _applyTargetRegistry.getApplyTarget)(op.model);
  const overlay = new Map();
  const rejectedReplacements = new Set();
  const rejectedReplaceSources = new Set();
  ...
  for (const row of allAccepted) {
    const target = (0, _applyTargetRegistry.getApplyTarget)(row.model);
    for (const scopeKey of target.reactiveScopes?.([row.id]) ?? []) {
      ...
      const groupKey = (0, _serialize.compositeKey)(row.model, scopeKey);
  ...
  for (const group of repositionGroups.values()) {
    const target = (0, _applyTargetRegistry.getApplyTarget)(group.model);
    const placements = target.planScopePlacement(group.scopeKey, [...group.ids], (model, id) => readPlannedRow(overlay, model, id));
    for (const placement of placements) {
      repositioned.set((0, _serialize.compositeKey)(group.model, group.scopeKey, placement.id), {
        kind: 'scope-delta',
        model: group.model,
  ...
  if (rejectedReplacements.size === 0) return {
    ops: planned,
    operationTransitions
  };
  ...
  for (const op of planned) {
    ...
    const orphaned = op.append.filter(entry => rejectedReplacements.has((0, _serialize.compositeKey)(op.model, entry.id)) && readPlannedRow(overlay, op.model, entry.id) === undefined);
    if (orphaned.length === 0) {
      gated.push(op);
      continue;
    }
    (0, _logger.getDbLogger)().error('membership append dropped for rejected replacement', {
      model: op.model,
      scopeKey: op.scopeKey,
      ids: orphaned.map(entry => entry.id)
    });
    (0, _diagnostics.noteDataLoss)('orphan-membership-dropped', op.model, orphaned.length);
