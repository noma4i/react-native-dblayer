...
const prepareOperations = (ops, overlay, rejectedReplacements) => {
  const preparedOps = [];
  const accepted = [];
  const destroyed = [];
  ...
  for (const op of ops) {
    ...
    if (op.kind === 'upsert') {
      ...
      for (const input of op.rows) {
        ...
        if (op.origin === 'replace' && inputId && !prepared) rejectedReplacements.add((0, _serialize.compositeKey)(op.model, inputId));
