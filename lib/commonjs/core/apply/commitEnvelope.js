...
const prepareOperations = (ops, overlay) => {
  ...
  for (const op of ops) {
    ...
    if (op.kind === 'counter') {
      ...
      if (!previous) {
        (0, _diagnostics.noteCounterOpDrop)();
        continue;
      }
    ...
    const entryBelongs = scopeKey => entry => {
    ...
    if (op.kind === 'scope') {
      preparedOps.push({
        ...
        next: {
          ...op.next,
          entries: op.next.entries.filter(entryBelongs(op.scopeKey))
        }
      });
      continue;
    }
    if (op.kind === 'scope-delta') {
      const append = (0, _scopeIndex.deduplicateScopeEntriesById)(op.append).filter(entryBelongs(op.scopeKey));
