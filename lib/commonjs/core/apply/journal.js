...
const checkpointEpoch = (storage, prefix) => {
  ...
  if (!raw) return 0;
  const decoded = (0, _persistenceCodec.decodePersistence)(
    raw,
    _persistenceCodec.PERSISTENCE_SCHEMA_VERSION,
    value => (0, _normalizeHelpers.isNonArrayRecord)(value) && (0, _normalizeHelpers.isNonNegativeSafeInteger)(value.lastCheckpointEpoch)
  );
...
const isValidJournalOp = value => {
  ...
  if (op.kind === 'upsert')
    return (
      ...
      op.rows.every(row => (0, _normalizeHelpers.isNonArrayRecord)(row) && (0, _normalizeHelpers.isNonEmptyString)(row.id)) &&
      (op.origin === undefined || op.origin === 'replace')
    );
  if (op.kind === 'destroy')
    return (
      Array.isArray(op.ids) &&
      op.ids.every(_normalizeHelpers.isNonEmptyString) &&
  ...
  if (op.kind === 'scope-delta')
    return (
      (0, _normalizeHelpers.isNonEmptyString)(op.scopeKey) &&
      (0, _scopeIndex.isScopeEntrySet)(op.append) &&
      Array.isArray(op.detach) &&
      op.detach.every(_normalizeHelpers.isNonEmptyString)
    );
  return false;
};
const isPersistedJournalRecord = value => {
  ...
  const record = value;
  return (
    (0, _normalizeHelpers.isNonEmptyString)(record.txId) &&
    (0, _normalizeHelpers.isPositiveSafeInteger)(record.runtimeEpoch) &&
    (0, _normalizeHelpers.isPositiveSafeInteger)(record.epoch) &&
    (record.status === 'pending' || record.status === 'committed') &&
    Array.isArray(record.ops) &&
    record.ops.every(op => (0, _normalizeHelpers.isNonArrayRecord)(op) && (0, _normalizeHelpers.isPositiveSafeInteger)(op.schemaVersion) && 'payload' in op)
  );
};
