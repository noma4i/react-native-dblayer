...
const readJournalRecord = (storage, prefix, journalKey) => {
  ...
  const decoded = (0, _persistenceCodec.decodeVersionedRecord)(raw, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, JOURNAL_RECORD_VERSION, value => isPersistedJournalRecord(value, journalEpoch));
  if (decoded.kind === 'unsupported') throw new Error(`Unsupported persistence schema version ${decoded.schemaVersion}`);
  if (decoded.kind === 'stale-version') {
    storage.set(journalKey, null);
    (0, _diagnostics.noteDataLoss)('journal-stale-version-drop', '__runtime__', 1);
    return null;
  }
