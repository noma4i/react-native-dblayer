...
exports.snapseqKey = exports.readSnapseq = exports.readDeltaLog = exports.highestPersistedSeq = exports.encodeDelta = exports.deltaKey = exports.decodeDelta = void 0;
...
const deltaKey = (prefix, seq) => `${prefix}delta:${String(seq).padStart(SEQ_WIDTH, '0')}`;
exports.deltaKey = deltaKey;
const snapseqKey = (prefix, model) => `${prefix}snapseq:${model}`;

/** Light delta codec: the ops already passed the lossless gate at plan time - one stringify pass, no checksum. */
exports.snapseqKey = snapseqKey;
const encodeDelta = (seq, ops) => JSON.stringify({
...
const readSnapseq = (storage, prefix, model) => {
 ...
 */
exports.readSnapseq = readSnapseq;
const readDeltaLog = (storage, prefix) => {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    if (decoded !== 'stale') {
      (0, _diagnostics.noteDataLoss)('delta-tail-cut', '__runtime__', cut.length);
      (0, _logger.getDbLogger)().error('delta tail cut', {
        from: keys[index],
        dropped: cut.length
      });
    }
...
exports.readDeltaLog = readDeltaLog;
const highestPersistedSeq = (storage, prefix) => {
  let highest = -1;
...
exports.highestPersistedSeq = highestPersistedSeq;
