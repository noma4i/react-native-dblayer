...
const prepareOperations = (ops, overlay) => {
  ...
  for (const op of ops) {
    ...
    if (op.kind === 'counter') {
      ...
      const prepared = target.preparePatch(
        op.id,
        {
    ...
    if (op.kind === 'scope-delta') {
      ...
      const placed =
        keyless.length > 0
          ? new Map(target.planScopePlacement(op.scopeKey, keyless, (model, id) => readPlannedRow(overlay, model, id)).map(entry => [entry.id, entry.orderKey]))
          : new Map();
      preparedOps.push({
        kind: 'scope-delta',
        model: op.model,
