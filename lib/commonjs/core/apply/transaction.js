 ...
 */
const createCommitEnvelope = (ops, extraEntries) => ({
  schemaVersion: 1,
  txId: `runtime:${(0, _configure.getRuntimeGeneration)()}`,
  epoch: (0, _configure.getRuntimeGeneration)(),
...
const recordCounterValues = ops => {
  ...
  return ops.map(op => {
    ...
    const key = (0, _serialize.compositeKey)(op.model, op.id, op.field);
...
const createApplyRuntime = options => {
  ...
  return {
    commit: envelope => {
      ...
      let batch;
      try {
        batch = (0, _store.runInApplyBatch)(() => applyPlan(recordedOps));
        (0, _store.syncStoreScopes)(batch, getApplyTarget, true);
      } catch (error) {
    ...
    replay: () => {
      ...
      for (const record of journal.allRecords()) {
        ...
        const batch = (0, _store.runInApplyBatch)(() => applyPlan(ops));
        (0, _store.syncStoreScopes)(batch, getApplyTarget);
        if (checkpoint) {
          storage.set(journal.committedEntry(record, checkpoint.flushedEpoch()));
