...
exports.createSubscriptionLifecycle = void 0;
...
const createSubscriptionLifecycle = entries => {
  ...
  const scheduleRetry = (state, epoch, token) => {
    ...
    if (!(0, _networkState.isFetchNetworkOnline)()) {
      const release = (0, _networkState.subscribeFetchNetwork)(() => {
        ...
        release();
        if (!(0, _networkState.isFetchNetworkOnline)() || !isCurrentAttempt(state, epoch, token) || state.unsubscribe) return;
...
exports.createSubscriptionLifecycle = createSubscriptionLifecycle;
