...
const createEntry = (modelId, scopeKey, sortMeta) => {
  ...
  const liveQuery = (0, _facade.createLiveQueryCollection)(query => {
    const joined = query
      ...
      .where(({ membership }) => (0, _facade.eq)(membership.scopeKey, scopeKey))
      .join(
        {
          entity: entities
        },
        ({ membership, entity }) => (0, _facade.eq)(membership.rowId, entity.id)
      );
  ...
  entry.scopeSubscription = (0, _configure.getCommitBus)().subscribeIncremental(
...
function useScopeLiveRows(modelId, scopeKey, sortMeta, isResolved, options = {}) {
  ...
  return (0, _scopeRetention.useScopeRetention)(
    scopeKey,
    {
      rows: store.rows,
