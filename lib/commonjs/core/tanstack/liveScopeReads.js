...
exports.getScopeLiveReadRegistryStats = getScopeLiveReadRegistryStats;
...
function getScopeLiveReadRegistryStats() {
  return {
 ...
 */
function useScopeLiveRows(modelId, scopeKey, sortMeta, options = {}) {
  ...
  const getSnapshot = (0, _react.useCallback)(() => gateRef.current.projectRows(scopeKey == null ? EMPTY_ROWS : entryFor(modelId, scopeKey, sortMeta).snapshot, optionsRef.current), [modelId, scopeKey, sortMeta]);
 ...
 */
function useScopeLiveWindowRows(modelId, scopeKey, sortMeta, windowSize, options = {}) {
  ...
  const getSnapshot = (0, _react.useCallback)(() => {
    ...
    if (previous.totalCount === source.length && (0, _useLiveRead.arraysShallowEqual)(previous.rows, rows)) {
      windowRef.current = {
        source,
        size: windowSize,
        snapshot: previous
