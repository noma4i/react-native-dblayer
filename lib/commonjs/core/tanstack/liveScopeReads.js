 ...
 */
function useScopeLiveRows(modelId, scopeKey, sortMeta) {
  const {
    entry,
    subscribe
  } = useScopeLiveEntry(modelId, scopeKey, sortMeta);
  const getSnapshot = (0, _react.useCallback)(() => scopeKey == null ? EMPTY_ROWS : entryFor(modelId, scopeKey, sortMeta).snapshot, [modelId, scopeKey, sortMeta]);
 ...
 */
function useScopeLiveWindowRows(modelId, scopeKey, sortMeta, windowSize) {
  const {
    subscribe
  } = useScopeLiveEntry(modelId, scopeKey, sortMeta);
