    ...
    (0, _facade.runInWriteBatch)(() => {
      for (const modelId of new Set([...rowIdsByModel.keys(), ...scopeKeysByModel.keys()])) {
        ...
        for (const id of rowIds) {
          ...
          if (!(0, _useLiveRead.rowsShallowEqual)(current, next)) {
            writer.write({
              type: `update`,
              value: next
            });
        ...
        for (const scopeKey of scopeKeys) {
          ...
          if (meta.kind === `field`) {
            if (structural.rebuild) {
              ...
              for (const row of expected) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: row
                });else if (!(0, _useLiveRead.rowsShallowEqual)(current, row)) membershipWriter.write({
                  type: `update`,
                  value: row
                });
              }
              ...
              for (const rowId of structural.appendIds) {
                ...
                if (!current) membershipWriter.write({
                  type: `insert`,
                  value: next
                });else if (!(0, _useLiveRead.rowsShallowEqual)(current, next)) membershipWriter.write({
                  type: `update`,
                  value: next
                });
              }
            ...
            for (const change of batch.rows) {
              ...
              if (!(0, _useLiveRead.rowsShallowEqual)(current, next)) membershipWriter.write({
                type: `update`,
                value: next
              });
            }
