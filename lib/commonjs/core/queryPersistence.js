...
const isQueryInvalidationRecord = value => (0, _normalizeHelpers.isRecord)(value) && value.recordVersion === QUERY_INVALIDATION_RECORD_VERSION && typeof value.revision === 'number' && Number.isSafeInteger(value.revision) && value.revision >= 0 && (0, _normalizeHelpers.isRecord)(value.identities) && Object.values(value.identities).every(revision => typeof revision === 'number' && Number.isSafeInteger(revision) && revision >= 0);
...
const readInvalidationRecord = family => {
  ...
  const decoded = (0, _persistenceCodec.decodePersistence)(raw, _persistenceCodec.PERSISTENCE_SCHEMA_VERSION, isQueryInvalidationRecord);
