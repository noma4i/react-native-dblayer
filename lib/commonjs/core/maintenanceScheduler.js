...
const hasDisappeared = row => {
  let target;
  try {
    target = (0, _applyTargetRegistry.getApplyTarget)(row.model);
  } catch {
    return false;
  }
...
const startMaintenanceScheduler = options => {
  ...
  let pressure = 0;
  const sweep = new _pacer.Debouncer(() => {
    try {
      (0, _gc.collectGarbage)();
    } catch (error) {
      (0, _logger.getDbLogger)().error('MaintenanceScheduler', 'garbage collection failed', {
        error
      });
    } finally {
      pressure = 0;
