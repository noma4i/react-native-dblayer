...
const collectGarbagePass = () => {
  ...
  const activeScopeDependencies = new Set((0, _configure.getCommitBus)().activeDependencies().filter(dependency => dependency.kind === 'scope').map(dependency => (0, _serialize.compositeKey)(dependency.model, dependency.scopeKey)));
  const now = Date.now();
...
const collectGarbage = () => (0, _store.runInStoreTransaction)(collectGarbagePass);
