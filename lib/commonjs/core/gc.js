...
const collectGarbage = () => {
  ...
  const activeScopeDependencies = new Set((0, _configure.getCommitBus)().activeDependencies().filter(dependency => dependency.kind === 'scope').map(dependency => `${dependency.model}\0${dependency.scopeKey}`));
  const now = Date.now();
