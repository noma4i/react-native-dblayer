...
exports.registerRelationHost = exports.references = exports.hasOne = exports.hasMany = exports.hasDependentCascade = exports.expandPlan = exports.belongsTo = void 0;
  ...
  const destroyEffects = (modelId, id, row) => {
    ...
    for (const relation of Object.values(host.relations())) {
      ...
      if (relation.kind === 'hasMany' && relation.dependent === 'destroy') {
        const overlayChildren = [...(overlay.get(relation.model.modelId)?.values() ?? [])].filter(child => child !== null && child[relation.foreignKey] === id);
        const ids = [...relation.model.getWhere({
          [relation.foreignKey]: id
        }), ...overlayChildren].map(child => String(child.id)).filter((childId, index, all) => all.indexOf(childId) === index).filter(childId => !destroyed.has(`${relation.model.modelId}:${childId}`));
