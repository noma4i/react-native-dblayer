  ...
  const destroyEffects = (modelId, id, row) => {
    ...
    for (const relation of Object.values(host.relations())) {
      ...
      if (relation.kind === 'hasMany' && relation.dependent === 'destroy') {
        ...
        const ids = [...liveChildren, ...overlayChildren].map(child => String(child.id)).filter((childId, index, all) => all.indexOf(childId) === index).filter(childId => !destroyed.has(`${relation.model.modelId}:${childId}`));
