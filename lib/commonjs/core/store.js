...
exports.storeScopeCollection =
  exports.runInApplyBatch =
  exports.restoreStoreReads =
  exports.resetStores =
  exports.registerModelStoreFactory =
  exports.publishProjectedBatch =
  exports.poisonStoreReads =
  exports.markStoresReady =
  exports.hydrateStoreScopes =
  exports.createModelStore =
    void 0;
...
const isTombstoneRecord = value =>
  (0, _normalizeHelpers.isNonArrayRecord)(value) &&
  Object.entries(value).every(
    ([id, tombstone]) =>
      (0, _normalizeHelpers.isNonEmptyString)(id) && (0, _normalizeHelpers.isNonArrayRecord)(tombstone) && (0, _normalizeHelpers.isNonNegativeSafeInteger)(tombstone.at)
  );
...
const createModelStore = options => {
  ...
  const buildScopeCollection = scopeKey =>
    (0, _db.createLiveQueryCollection)({
      ...
      query: q =>
        q
          ...
          .where(({ membership }) => (0, _db.eq)(membership.scopeKey, scopeKey))
          .join(
            {
              entity: entities
            },
            ({ membership, entity }) => (0, _db.eq)(membership.entityId, entity.id)
          )
          .orderBy(({ membership }) => membership.orderKey, {
            direction: 'asc',
            stringSort: 'lexical'
          })
  ...
  const projectScopeChange = change => {
    ...
    for (const entityId of change.detachIds ?? []) {
      if (memberships.has(membershipKey(change.scopeKey, entityId)))
        messages.push({
          type: 'delete',
          key: membershipKey(change.scopeKey, entityId)
        });
  ...
  const previewUpsert = (incoming, upsertOptions) => {
    ...
    if (
      ...
      changedFields.every(field => (0, _serialize.stableSerialize)(previous[field]) === (0, _serialize.stableSerialize)(row[field]))
    ) {
  ...
  const store = {
    ...
    upsert: (incoming, upsertOptions = {}) => {
      ...
      if (prepared.changedFields !== null && prepared.changedFields.length === 0)
        return {
...
const hydrateStoreScopes = sources => {
  for (const [model, source] of sources) {
    ...
    store.applyScopeChanges(
      source.readAllScopeKeys().map(scopeKey => ({
        scopeKey,
