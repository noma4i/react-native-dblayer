...
const useDbInfiniteRequest = config => {
  ...
  const baseConfig = (0, _react.useMemo)(() => {
    ...
    return {
      ...
      queryFn: ({
        pageParam
      }) => (0, _requestRuntime.executeDbInfiniteRequest)(configRef.current, pageParam, patchStateRef.current),
      extract,
      inactive: config.inactive,
      ...(config.getCursor ? {
  ...
  }, [config.direction, config.emptyStaleTime, config.enabled, config.gcTime, config.getCursor, config.inactive, keySignature, config.query, config.read, config.readMode, config.refetchOnMount, config.resolveSyncContract, config.scope, config.staleTime]);
