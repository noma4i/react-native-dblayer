...
const useBaseInfiniteQuery = config => {
  ...
  const fetchState = (0, _react.useMemo)(() => isInactive ? null : config.collection.getFetchState?.(filter) ?? null, [config.collection, filter, isInactive]);
  const skipInitialFetch = (0, _react.useMemo)(() => isInactive ? false : config.collection.shouldSkipInitialFetch(filter, config.staleTime), [config.collection, config.staleTime, filter, isInactive]);
