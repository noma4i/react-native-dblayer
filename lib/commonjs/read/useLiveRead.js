...
const depsSignature = deps => deps.map(dep => dep.kind === 'model' ? (0, _serialize.compositeKey)('m', dep.model) : dep.kind === 'scope' ? (0, _serialize.compositeKey)('s', dep.model, dep.scopeKey) : dep.kind === 'pending' ? (0, _serialize.compositeKey)('p', dep.model, dep.id) : (0, _serialize.compositeKey)('r', dep.model, dep.id, dep.fields?.join(',') ?? '')).join('|');
...
const useLiveRead = (compute, deps, isEqual = Object.is) => {
  ...
  (0, _react.useSyncExternalStore)(subscribe, () => state.version, () => state.version);
  return state.value;
};
