...
const createModelReadEngine = options => {
  ...
  engine.apply = batch => {
    ...
    if (requiresRebuild) {
      ...
      (0, _diagnostics.noteReadEngineApply)('rebuild', relevant.length, (globalThis.performance?.now?.() ?? Date.now()) - startedAt);
      return true;
    }
    ...
    if (!changed) {
      (0, _diagnostics.noteReadEngineApply)('delta', relevant.length, (globalThis.performance?.now?.() ?? Date.now()) - startedAt);
      return false;
    }
    const previous = engine.value;
