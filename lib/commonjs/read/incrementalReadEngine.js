...
exports.useIncrementalRead = exports.sortModelReadRows = exports.incrementalSignature = exports.createScopeReadEngine = exports.createModelReadEngine = void 0;
...
const engineValuesEqual = (left, right) => Array.isArray(left) && Array.isArray(right) ? left.length === right.length && left.every((value, index) => Object.is(value, right[index])) : Object.is(left, right);
...
const createModelReadEngine = options => {
  ...
  engine.apply = batch => {
    ...
    if (requiresRebuild) {
      ...
      rebuild();
      if (!engineValuesEqual(previous, engine.value)) engine.version += 1;else engine.value = previous;
      return true;
    }
...
const createScopeReadEngine = options => {
  ...
  const render = () => {
    ...
    if (sort && sort !== 'server-order') {
      if ('comparator' in sort) next.sort(sort.comparator);else next.sort((left, right) => compareField(left, right, sort.field, sort.direction, ordinals));
    }
    engine.value = next;
  };
  const rebuild = () => {
