...
const createModelReadEngine = options => {
  ...
  const render = () => {
    if (rows) {
      ...
      ordered = orderBy.length > 0 ? sortModelReadRows(values, orderBy, options.options?.limit) : options.options?.limit === undefined ? values : values.slice(0, Math.max(0, options.options.limit));
