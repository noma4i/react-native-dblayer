...
var _storeDerivedCollections = require("../core/storeDerivedCollections.js");
...
const createScopeReadHolder = (modelId, scopeKey, seed) => {
  ...
  const source = (0, _store.storeScopeCollection)(modelId, scopeKey);
  const listeners = new Set();
  let resolvedEntries = [];
  let revision = (0, _applyTargetRegistry.getApplyTarget)(modelId).readScopeOrderRevision(scopeKey);
  ...
  const rowCompare = (0, _applyTargetRegistry.getApplyTarget)(modelId).compareScopeRows(scopeKey);
  ...
  const commitSubscription = (0, _configure.getCommitBus)().subscribeIncremental(() => {
 ...
 */
const holders = (0, _storeDerivedCollections.createDerivedCollectionCache)('scopeReadHolders');
const holderKey = (modelId, scopeKey, sortMeta) => (0, _readIdentity.incrementalSignature)('scope-read', modelId, scopeKey, sortMeta) + `|g${(0, _configure.getRuntimeGeneration)()}`;
 ...
 */
const seeds = new Map();
(0, _reset.registerReset)(() => seeds.clear());
...
const readSeed = (key, modelId, scopeKey) => {
  ...
  const rowCompare = (0, _applyTargetRegistry.getApplyTarget)(modelId).compareScopeRows(scopeKey);
  const entries = (0, _store.storeScopeCollection)(modelId, scopeKey).toArray().filter(isScopeRow).map(entry => {
    const next = stripEngineFields(entry);
    const known = previousById.get(next.id);
    const row = known && (0, _rowEquality.rowsShallowEqual)(known, next) ? known : next;
  ...
  if (rowCompare) entries.sort((left, right) => rowCompare(left.row, right.row));else entries.sort((left, right) => (0, _serialize.compareCodepoints)(left.source.orderKey, right.source.orderKey) || (0, _serialize.compareCodepoints)(left.source.id, right.source.id));
