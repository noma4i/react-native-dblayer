...
const validateProjectionOptions = (options, surface, validation) => {
...
const createProjectionGate = () => {
  ...
  let previousOptionsSignature = null;
  return {
    projectValue(id, source, output, renderKeys) {
      ...
      if (current && (0, _rowEquality.rowsShallowEqual)(current.equalityValue, nextEqualityValue)) {
        entries.set(id, {
          source,
    ...
    projectRows(rows, options) {
      if (previousSource === rows && previousOptionsSignature !== null && previousOptionsSignature.select === options.select && (previousOptionsSignature.renderKeys === options.renderKeys || (0, _arrayEquality.arraysShallowEqual)(previousOptionsSignature.renderKeys ?? [], options.renderKeys ?? []))) {
        return previousRows;
      }
      ...
      if ((0, _arrayEquality.arraysShallowEqual)(previousRows, next)) return previousRows;
      previousRows = next;
      return next;
    }
  };
...
const useProjectedLiveRows = (compute, deps, options, surface) => {
  ...
  const gateRef = (0, _react.useRef)(createProjectionGate());
  optionsRef.current = options;
  return (0, _useLiveRead.useLiveRead)(() => gateRef.current.projectRows(compute(), optionsRef.current), deps, _arrayEquality.arraysShallowEqual);
};
