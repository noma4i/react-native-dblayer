...
const createProjectionGate = () => {
  ...
  return {
    ...
    projectRows(rows, options) {
      if (previousSource === rows && previousOptionsSignature !== null && previousOptionsSignature.select === options.select && (previousOptionsSignature.renderKeys === options.renderKeys || (0, _useLiveRead.arraysShallowEqual)(previousOptionsSignature.renderKeys ?? [], options.renderKeys ?? []))) {
        return previousRows;
      }
