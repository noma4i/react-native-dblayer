...
const createMutationRuntime = config => {
  const runtime = {
    config,
  ...
  if (config.optimistic && !(0, _mutationConfiguration.isMethodOptimistic)(config.optimistic) && !(0, _mutationConfiguration.isRespondOptimistic)(config.optimistic) && config.optimistic.correlate) {
    (0, _mutationCorrelation.registerMutationCorrelator)(config.optimistic.model.modelId, config.optimistic.correlate);
  }
    ...
    try {
      if (optimistic && (0, _mutationConfiguration.isRespondOptimistic)(optimistic)) {
        ...
        if (optimisticOps.length > 0) {
          ...
          (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(optimisticOps, () => operations.persistEntries()));
        } else {
          operations.begin(beginFields);
        }
      ...
      if (tracked) {
        if (commitOps.length > 0) {
          ...
          (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(commitOps, () => operations.persistEntries()));
        } else {
          operations.close(operationId, 'committed');
        }
      ...
      if (tracked) {
        ...
        if (rollbackOps.length > 0) {
          ...
          (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(rollbackOps, () => operations.persistEntries()));
        } else {
          operations.close(operationId, status);
        }
      }
