...
const createMutationRuntime = (config, definitionId) => {
  const mapExactVariables = config[_mutationVariables.exactMutationVariables];
  const runtime = {
    config,
  ...
  const runWithTempId = async (input, forcedTempId) => {
    ...
    const persistedFailedInput = input !== undefined && optimistic && !(0, _mutationConfiguration.isMethodOptimistic)(optimistic) && !(0, _mutationConfiguration.isRespondOptimistic)(optimistic) ? (0, _operationState.serializeOperationInput)(input) : null;
    ...
    try {
      ...
      ops.push(...(config[_mutationRootPlan.exactMutationRootPlan]?.({
        data
      }) ?? []));
      if (!generationFence.isCurrent()) return null;
      ...
      if (tracked) {
        if (commitOps.length > 0) {
          (0, _configure.getApplyRuntime)().commit((0, _commitEnvelope.createCommitEnvelope)(commitOps, [{
            kind: 'close',
            operationId,
            status: 'committed'
          }]));
    ...
    if (!(0, _writePlan.runWritePlanInvalidations)(plannedInvalidations, generationFence.isCurrent, error => reportCallbackError(error, 'write.invalidate'))) return null;
    if (!generationFence.isCurrent()) return null;
