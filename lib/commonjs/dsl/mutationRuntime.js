...
const createMutationRuntime = (config, definitionId) => {
  ...
  if (
    config.optimistic &&
    !(0, _mutationConfiguration.isMethodOptimistic)(config.optimistic) &&
    !(0, _mutationConfiguration.isRespondOptimistic)(config.optimistic) &&
    config.optimistic.correlate
  ) {
  ...
  const runWithTempId = async (input, forcedTempId) => {
    ...
    const persistedFailedInput =
      optimistic && !(0, _mutationConfiguration.isMethodOptimistic)(optimistic) && !(0, _mutationConfiguration.isRespondOptimistic)(optimistic)
        ? (0, _operationState.serializeOperationInput)(input)
        : null;
    ...
    try {
      if (optimistic && (0, _mutationConfiguration.isRespondOptimistic)(optimistic)) {
        ...
        respondInverse = inverseFromRespond(
          fabricated,
          {
            tempId,
            operationId
        ...
        const optimisticOps = planFromRespond(
          fabricated,
          {
            tempId,
            operationId
        ...
        if (optimisticOps.length > 0) {
          (0, _configure.getApplyRuntime)().commit(
            (0, _transaction.createCommitEnvelope)(optimisticOps, [
              {
                kind: 'begin',
        ...
        if (reuseId != null && (forcedTempId != null || optimistic.model.find(reuseId) !== undefined)) {
          ...
          (0, _configure.getApplyRuntime)().commit(
            (0, _transaction.createCommitEnvelope)(ops, [
              {
                kind: 'begin',
                operation: beginFields
              }
        ...
        (0, _configure.getApplyRuntime)().commit(
          (0, _transaction.createCommitEnvelope)(
            [
              {
                kind: 'patch',
                model: optimistic.model.modelId,
      ...
      while (true) {
        ...
        try {
          data = (0, _transport.responseDataOrThrow)(
            await (0, _configure.getDbRuntimeConfig)().transport.mutation({
      ...
      const commitOps = methodPatchOptimistic
        ? ops.map(op =>
            op.kind === 'upsert' && op.model === optimistic.model.modelId
              ? {
                  ...op,
                  operationId
                }
      ...
      if (tracked) {
        if (commitOps.length > 0) {
          (0, _configure.getApplyRuntime)().commit(
            (0, _transaction.createCommitEnvelope)(commitOps, [
              {
                kind: 'close',
                operationId,
                status: 'committed'
      ...
      if (tracked) {
        const status =
          optimistic &&
          !(0, _mutationConfiguration.isMethodOptimistic)(optimistic) &&
          !(0, _mutationConfiguration.isRespondOptimistic)(optimistic) &&
          optimistic.failure !== 'rollback'
            ? 'failed'
            : 'rolledback';
        if (rollbackOps.length > 0) {
          (0, _configure.getApplyRuntime)().commit(
            (0, _transaction.createCommitEnvelope)(rollbackOps, [
              {
                kind: 'close',
                operationId,
                status
  ...
  const retry = async tempId => {
    ...
    if (
      input === undefined ||
      !optimisticConfig ||
      (0, _mutationConfiguration.isMethodOptimistic)(optimisticConfig) ||
      (0, _mutationConfiguration.isRespondOptimistic)(optimisticConfig)
    )
      return null;
