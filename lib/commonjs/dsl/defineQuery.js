...
const defineQuery = config => {
  ...
  const useObservedState = key => {
    ...
    const getSnapshot = (0, _react.useCallback)(() => `${observer.getCurrentResult().fetchStatus}:${observer.getCurrentResult().status}:${observer.getCurrentResult().failureCount}:${observer.getCurrentResult().dataUpdatedAt}:${localVersions.get(key) ?? 0}`, [key, observer]);
