...
const defineQuery = config => {
  ...
  const registerScope = scope => {
    ...
    if (isScopeDestination(config.into)) (0, _internalHandles.getInternalScopeHandle)(config.into).key(scope);
  ...
  const applyResponse = (scope, data, resetOrder, resurrectDestroyed) => {
    ...
    const ops = [];
    const rows = isScopeDestination(config.into)
      ? pairs.map(pair => ({
          row: pair.node,
    ...
    if (isScopeDestination(config.into))
      ops.push(
        ...(0, _internalHandles.getInternalScopeHandle)(config.into).planApply(scope, rows, coverage, {
          resetOrder
        })
      );
    else
      ops.push(
        ...(0, _internalHandles.getInternalModelHandle)(config.into).planRows(
          nodes,
          resurrectDestroyed
            ? {
                origin: 'event'
    ...
    if (ops.length > 0) (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(ops));
    const committedRows = isScopeDestination(config.into) ? rows.map(entry => entry.row) : nodes;
    const ids = committedRows.flatMap(row => ((0, _normalizeHelpers.isRecord)(row) && row.id != null ? [(0, _serialize.compositeKey)(destinationModelId, String(row.id))] : []));
    return {
  ...
  const execute = async (scope, key, resurrectDestroyed, context) => {
    ...
    const guardKey = isScopeDestination(config.into)
      ? (0, _serialize.compositeKey)(destinationModelId, (0, _internalHandles.getInternalScopeHandle)(config.into).key(scope))
      : (0, _serialize.compositeKey)(keyName, scopeKey);
    ...
    let data;
    try {
      data = (0, _transport.responseDataOrThrow)(
        await (0, _configure.getDbRuntimeConfig)().transport.query({
    ...
    const previousPages = (0, _configure.getDbQueryClient)().getQueryData(queryKeyOf(key))?.pages ?? 0;
  ...
  const run = async (scope, options) => {
      ...
      await client.fetchQuery({
        ...
        queryFn: async () => {
          ...
          const meta = await execute(scope, key, options.resurrectDestroyed === true, {
            cursor,
            isCurrent: () => (0, _configure.getRuntimeGeneration)() === generation
          });
          if (meta === null)
            return (
    ...
    if ((0, _configure.getRuntimeGeneration)() !== generation) return;
    setLocalState(key, {
      isPaused: false,
      isFetchingNextPage: false
    });
  ...
  if (destinationModelId) (0, _invalidationRegistry.registerModelInvalidation)(destinationModelId, keyName, scope => invalidate(scope));
  ...
  const useReader = (scope, enabled, resurrectDestroyed, forceAbsentRefetch) => {
    ...
    (0, _react.useEffect)(() => {
      ...
      const unsubscribeOnline = (0, _networkState.subscribeFetchNetwork)(() => {
        ...
        if (!(0, _queryFreshness.isQueryFresh)(client, queryKey, staleTimeOf(key)))
          void run(scope, {
            restart: false,
            resurrectDestroyed
          }).catch(() => {});
  ...
  const buildResult = (rows, enabled, state, scope) => {
    ...
    return {
      ...
      fetchNextPage: () => {
        if (scope !== null && config.page && state.hasNextPage && !state.isFetching)
          void run(scope, {
            restart: false,
            nextPage: true
          }).catch(() => {});
