...
const defineQuery = rawConfig => {
  ...
  const persist = (scope, meta, invalidationRevision, onError) => {
    ...
    const queryState = (0, _configure.getDbQueryClient)().getQueryState(queryKeyOf(identity));
  ...
  const landingError = message => (0, _syncError.reportSyncError)(new Error(message), {
    source: 'query',
    model: destinationModelId,
    key: keyName
  }, 'defineQuery');
  ...
  const pageMetaOf = connection => {
    ...
    if (!(0, _normalizeHelpers.isRecord)(info)) throw landingError('react-native-dblayer: connection landing has no usable pageInfo');
  ...
  const applyResponse = (scope, data, resetOrder, resurrectDestroyed, baseRevision, isCurrent) => {
    ...
    const ids = admitted.map(entry => (0, _serialize.compositeKey)(destinationModelId, entry.id));
    return {
      meta: pageMeta,
      ids,
  ...
  const execute = async (scope, key, resurrectDestroyed, context) => {
    ...
    const guardKey = isScopeDestination(config.into) ? (0, _serialize.compositeKey)(keyName, destinationModelId, (0, _internalHandles.getInternalScopeHandle)(config.into).key(scope)) : (0, _serialize.compositeKey)(keyName, scopeKey);
      ...
      const meta = await client.fetchQuery({
        ...
        queryFn: async () => {
          ...
          if (meta === null) {
            ...
            if (cached === undefined) throw new _reactQuery.CancelledError();
            return cached;
          }
          return meta;
        },
  ...
  const readDestinationRows = rawScope => {
    ...
    const rows = (meta?.ids ?? []).flatMap(id => {
      const row = destination.find((0, _serialize.parseCompositeKey)(id)[1]);
