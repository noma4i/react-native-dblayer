...
const defineQuery = config => {
  ...
  const committedRowsKey = scopeKey => (0, _serialize.compositeKey)(keyName, scopeKey);
  const staleGuardKey = (scope, scopeKey) => isScopeDestination(config.into) ? (0, _serialize.compositeKey)(String(config.into.modelId ?? keyName), (0, _internalHandles.getInternalScopeHandle)(config.into).key(scope)) : (0, _serialize.compositeKey)(keyName, scopeKey);
  ...
  const committedIdsOf = rows => rows.flatMap(row => (0, _normalizeHelpers.isRecord)(row) && row.id != null ? [String(row.id)] : []);
  ...
  const applyResponse = (scope, data, resetOrder, resurrectDestroyed) => {
    ...
    if (isScopeDestination(config.into)) {
      ...
      ops.push(...(0, _internalHandles.getInternalModelHandle)(config.into).planRows(nodes, resurrectDestroyed ? {
        origin: 'event'
      } : undefined));
  ...
  const runFetch = async (scope, cursor, resurrectDestroyed = false) => {
    ...
    if (isReset) {
      if (issueSeq < appliedReset) return {
        endCursor: null,
        hasNextPage: false,
        count: 0
      };
  ...
  const buildResult = (rows, flags) => {
    ...
    const phase = (0, _loadingState.computePhase)(phaseInput);
    return {
      data: rows,
      loadingState: (0, _loadingState.computeLoadingState)(phase, phaseInput),
      error: flags.error,
  ...
  const useInfiniteResult = (scope, options) => {
    ...
    const request = (0, _reactQuery.useInfiniteQuery)({
      ...
      ...(config.resumeStaleTime !== undefined ? {
        meta: {
          resumeStaleTime: config.resumeStaleTime
        }
      } : {})
    });
    const rows = useDestinationRows(scope);
    const hasRows = Array.isArray(rows) ? rows.length > 0 : rows !== undefined;
    const committedRowsDied = !hasRows && isScopeDestination(config.into) && !rowsSurvive((0, _compileDbWhere.buildScopeKey)(scope));
    (0, _react.useEffect)(() => {
  ...
  const useRowEnsured = (scope, rowId, readOpts) => {
    ...
    const ensureKey = (0, _serialize.compositeKey)((0, _compileDbWhere.buildScopeKey)(scope), rowId ?? '');
    const refetchedForAbsenceRef = (0, _react.useRef)(false);
    const ensureKeyRef = (0, _react.useRef)(ensureKey);
