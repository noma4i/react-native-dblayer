...
const defineQuery = rawConfig => {
  ...
  const useObservedState = (key, scope, enabled, resurrectDestroyed) => {
    ...
    // private one.
    const landed = client.getQueryData(queryKeyOf(key)) !== undefined;
    const observerOptions = {
      ...
      queryFn: async () => {
        ...
        if (scope === null || !(0, _networkState.isFetchNetworkOnline)()) return current();
        const fence = (0, _runtimeGeneration.createGenerationFence)();
        const meta = await execute(scope, key, resurrectDestroyed, {
          cursor: null,
    ...
    if (observerRef.current === null || observerRef.current.key !== key || observerRef.current.generation !== generation) {
      observerRef.current = {
        key,
        generation,
        observer: new _reactQuery.QueryObserver(client, observerOptions)
      };
    } else {
