...
const nodesOf = value => {
  ...
  if (!(0, _normalizeHelpers.isRecord)(value)) return value == null ? [] : [value];
...
const defineQuery = rawConfig => {
  ...
  const staleTimeOf = key => {
    ...
    return meta?.lastCount === 0 && (config.emptyStaleTime ?? defaults.emptyStaleTime) != null ? config.emptyStaleTime ?? defaults.emptyStaleTime : (0, _queryFreshness.resolveStaleTime)(config.staleTime, defaults) ?? defaults.staleTime ?? 0;
  };
  ...
  const useDestinationRows = isScopeDestination(config.into) ? scope => config.into.use(scope) : (_scope, state) => {
    ...
    const rowIds = state.ids.flatMap(id => {
      const parts = (0, _serialize.parseCompositeKey)(id);
  ...
  if (isScopeDestination(config.into)) {
    ...
    const useWindow = (rawScope, options) => {
      ...
      debouncerRef.current ??= new _pacer.Debouncer(() => advanceRef.current(), {
        wait: options?.loadMoreDebounceMs ?? 160
      });
      (0, _react.useEffect)(() => () => debouncerRef.current?.cancel(), []);
      const loadMore = (0, _react.useCallback)(() => debouncerRef.current?.maybeExecute(), []);
      return {
        ...bridge,
        loadMore
      };
