...
const createModelNormalization = config => {
  ...
  const normalize = (input, complete = false) => {
    ...
    const id = _fieldCodec.scalarFieldCodecs.id.read(config.rowId?.(input) ?? ((0, _normalizeHelpers.isRecord)(input) ? input.id : undefined));
  ...
  const admitPlanRow = value => {
    try {
      ...
      (0, _quarantine.putQuarantine)({
        kind: 'row',
        model: config.id,
        id: (0, _normalizeHelpers.isRecord)(value) && value.id !== undefined ? String(value.id) : '',
        raw: value,
