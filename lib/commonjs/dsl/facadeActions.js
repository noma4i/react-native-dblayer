...
var _optimisticInsertPlanners = require("./optimisticInsertPlanners.js");
...
var _requestRollback = require("../core/requestRollback.js");
...
const createAction = (runtime, name, definition) => {
  ...
  if (definition.mode === 'durable') {
    ...
    const runExecution = async (record, tempId, transportInput) => {
      ...
      try {
        ...
        if (active) {
          const status = error instanceof _mutationDeliveryError.MutationDeliveryUnknownError ? 'delivery_unknown' : 'failed';
          (0, _configure.getApplyRuntime)().commit((0, _commitEnvelope.createCommitEnvelope)([], [{
            kind: 'close',
            operationId,
            status
  ...
  const deliveryUnknownRequestRecord = rowId => (0, _configure.getOperationState)().deliveryUnknownForRow(runtime.modelId, rowId).find(record => record.actionMode === 'request' && record.actionKey === actionKey);
  const idempotencyKeyFor = (input, operationId) => {
    ...
    return {
      key: (0, _serialize.compositeKey)(actionKey, key),
      deduped: true
    };
  };
  ...
  if (requestDefinition.optimistic && (0, _modelRootPlan.modelRootIntentOf)(requestDefinition.optimistic.root) === 'insert') {
    (0, _optimisticInsertPlanners.registerOptimisticInsertPlanner)(runtime.modelId, actionKey, (input, tempId, operationId) => buildOptimisticPlan(input, tempId, operationId, false).ops);
  }
  const runRequestExecution = async (operationId, tempId, input, tracked) => {
    ...
    try {
      ...
      if (active) {
        if (error instanceof _mutationDeliveryError.MutationDeliveryUnknownError) {
          (0, _configure.getApplyRuntime)().commit((0, _commitEnvelope.createCommitEnvelope)([], [{
            kind: 'close',
            operationId,
          ...
          const planned = (0, _requestRollback.planRequestFailureRollback)(active, id => (0, _internalHandles.getInternalModelHandle)(runtime).readRow(id), (row, memberships) => (0, _internalHandles.getInternalModelHandle)(runtime).planRestore(row, memberships));
          (0, _configure.getApplyRuntime)().commit((0, _commitEnvelope.createCommitEnvelope)(planned.ops, [planned.transition]));
        }
      }
      if (generationFence.isCurrent()) {
        try {
