"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.defineView = void 0;
var _react = require("react");
var _diagnostics = require("../core/diagnostics.js");
var _reset = require("../core/reset.js");
var _serialize = require("../core/serialize.js");
var _useLiveRead = require("../read/useLiveRead.js");
var _rowEquality = require("../utils/rowEquality.js");
var _arrayEquality = require("../utils/arrayEquality.js");
var _ordering = require("../core/ordering.js");
var _projectionGate = require("../read/projectionGate.js");
var _scopeRetention = require("../read/scopeRetention.js");
var _requireFields = require("../read/requireFields.js");
var _configure = require("./configure.js");
var _internalHandles = require("../core/internalHandles.js");
const foreignKeyIndexes = new Map();
const foreignKeyIndexKey = (targetModelId, foreignKey) => (0, _serialize.compositeKey)(targetModelId, foreignKey);
const removeIndexedRow = (index, foreignKey, id) => {
  const bucket = index.rowsByForeignKey.get(foreignKey);
  const position = bucket.findIndex(row => row.id === id);
  if (position >= 0) bucket.splice(position, 1);
  if (bucket.length === 0) index.rowsByForeignKey.delete(foreignKey);
};
const foreignKeyIndexFor = (target, foreignKey) => {
  const key = foreignKeyIndexKey(target.modelId, foreignKey);
  const existing = foreignKeyIndexes.get(key);
  if (existing) return existing;
  const rowsByForeignKey = new Map();
  const fkById = new Map();
  for (const row of target.all()) {
    const value = row[foreignKey];
    if (typeof value !== 'string') continue;
    const bucket = rowsByForeignKey.get(value) ?? [];
    bucket.push(row);
    rowsByForeignKey.set(value, bucket);
    fkById.set(row.id, value);
  }
  const indexData = {
    rowsByForeignKey,
    fkById
  };
  const unsubscribe = (0, _configure.getCommitBus)().subscribeAll(batch => {
    let updates = 0;
    for (const change of batch.rows) {
      if (change.model !== target.modelId) continue;
      const previousForeignKey = indexData.fkById.get(change.id);
      const next = target.find(change.id);
      const nextForeignKey = typeof next?.[foreignKey] === 'string' ? next[foreignKey] : undefined;
      if (previousForeignKey && previousForeignKey !== nextForeignKey) removeIndexedRow(indexData, previousForeignKey, change.id);
      if (!nextForeignKey || !next) {
        indexData.fkById.delete(change.id);
        updates += 1;
        continue;
      }
      const bucket = indexData.rowsByForeignKey.get(nextForeignKey) ?? [];
      const position = bucket.findIndex(row => row.id === change.id);
      if (position >= 0) bucket[position] = next;else bucket.push(next);
      indexData.rowsByForeignKey.set(nextForeignKey, bucket);
      indexData.fkById.set(change.id, nextForeignKey);
      updates += 1;
    }
    if (updates > 0) (0, _diagnostics.noteFkIndex)('incremental', updates);
  });
  const index = {
    ...indexData,
    unsubscribe
  };
  foreignKeyIndexes.set(key, index);
  (0, _diagnostics.noteFkIndex)('full', rowsByForeignKey.size);
  return index;
};
(0, _reset.registerReset)(() => {
  for (const index of foreignKeyIndexes.values()) index.unsubscribe();
  foreignKeyIndexes.clear();
});
const idsOf = value => (Array.isArray(value) ? value : value == null ? [] : [value]).filter(id => typeof id === 'string' && id.length > 0);
const resolveRelation = (row, relation, rowsFor) => {
  if (relation.kind === 'belongsTo') {
    const id = row[relation.foreignKey];
    return typeof id === 'string' ? relation.model.find(id) ?? null : null;
  }
  const rows = rowsFor(relation.foreignKey, row.id);
  if (relation.kind === 'hasMany') return rows;
  return (0, _ordering.pickLowestRow)(rows, relation.comparator) ?? null;
};

/** Normalize the public generic contract once; runtime view evaluation intentionally remains row-shape agnostic. */
const normalizeViewConfig = config => config;

/**
 * Compose a model scope with declared relations or computed target ids into one pinpoint-reactive view.
 *
 * @param model Source model that owns the named scope and declared relation includes.
 * @param name Stable view name used in validation errors.
 * @param config Source scope, include declarations, projection, and optional render identity keys.
 * @returns A hook handle with full-scope and local-window reads.
 */
const defineView = (model, name, publicConfig) => {
  const config = normalizeViewConfig(publicConfig);
  const source = typeof config.source === 'string' ? model.scopes[config.source] : config.source;
  if (!source || source.modelId !== model.modelId) throw new Error(`${model.modelId} has no scope ${typeof config.source === 'string' ? config.source : name} for view ${name}`);
  (0, _projectionGate.validateProjectionOptions)(config, `${model.modelId}.view.${name}`, {
    allowCombined: true
  });
  const relations = (0, _internalHandles.getInternalModelHandle)(model).relations();
  const sourceInternal = (0, _internalHandles.getInternalScopeHandle)(source);
  for (const [alias, include] of Object.entries(config.include)) {
    const relationName = typeof include === 'string' ? include : Array.isArray(include) ? null : 'model' in include ? null : alias;
    if (relationName && !relations[relationName]) throw new Error(`${model.modelId} has no relation ${relationName} for view ${name}.${alias}`);
    if (relationName && relations[relationName].kind === 'references') throw new Error(`Model.view does not support references includes`);
  }
  const useItems = (scopeValue, limit) => {
    const cacheRef = (0, _react.useRef)(new Map());
    const projectionGateRef = (0, _react.useRef)((0, _projectionGate.createProjectionGate)());
    const includeProjectionGatesRef = (0, _react.useRef)(new Map());
    const evaluatedRef = (0, _react.useRef)(null);
    const projectIncludedRow = (alias, row, renderKeys) => {
      if (!renderKeys) return row;
      let gate = includeProjectionGatesRef.current.get(alias);
      if (!gate) {
        gate = (0, _projectionGate.createProjectionGate)();
        includeProjectionGatesRef.current.set(alias, gate);
      }
      return gate.projectValue(row.id, row, row, renderKeys);
    };
    const rowsFor = (relation, foreignKey, id) => {
      const index = foreignKeyIndexFor(relation.model, foreignKey);
      // Callers transform results immediately and must not retain these mutable bucket arrays.
      return index.rowsByForeignKey.get(id) ?? [];
    };
    const scopeKey = scopeValue == null ? null : sourceInternal.key(scopeValue);
    (0, _react.useEffect)(() => {
      if (scopeKey != null) sourceInternal.noteAccess(scopeValue);
    }, [scopeKey, scopeValue]);
    const evaluate = () => {
      const rows = scopeValue == null ? [] : sourceInternal.readRows(scopeValue);
      // filter/sort derive their own row set: every row must be materialized before the window slices.
      const derivesOwnSet = config.filter !== undefined || config.sort !== undefined;
      const visibleRows = derivesOwnSet || limit === null ? rows : rows.slice(0, limit);
      const relationModelIds = new Set();
      for (const [alias, include] of Object.entries(config.include)) {
        const relationName = typeof include === 'string' ? include : Array.isArray(include) ? null : 'model' in include ? null : alias;
        if (!relationName) continue;
        const relation = relations[relationName];
        if (relation.kind === 'hasMany' || relation.kind === 'hasOne') relationModelIds.add(relation.model.modelId);
      }
      const deps = [...[...relationModelIds].map(model => ({
        kind: 'model',
        model
      })), ...(scopeValue == null ? [] : [{
        kind: 'scope',
        model: source.modelId,
        scopeKey: sourceInternal.key(scopeValue)
      }])];
      const liveIds = new Set(rows.map(row => row.id));
      const projected = visibleRows.map((row, index) => {
        deps.push({
          kind: 'row',
          model: model.modelId,
          id: row.id
        });
        const included = {};
        for (const [alias, include] of Object.entries(config.include)) {
          const relationName = typeof include === 'string' ? include : Array.isArray(include) ? null : 'model' in include ? null : alias;
          const includeRenderKeys = typeof include === 'object' && !Array.isArray(include) ? include.renderKeys : undefined;
          if (relationName) {
            const relation = relations[relationName];
            const required = typeof include === 'object' && !Array.isArray(include) ? include.require : undefined;
            const resolved = resolveRelation(row, relation, (foreignKey, id) => rowsFor(relation, foreignKey, id));
            included[alias] = Array.isArray(resolved) ? resolved.filter(candidate => (0, _requireFields.hasRequiredFields)(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys)) : (0, _requireFields.hasRequiredFields)(resolved, required ?? []) ? projectIncludedRow(alias, resolved, includeRenderKeys) : null;
            if (relation.kind === 'belongsTo') {
              const id = row[relation.foreignKey];
              if (typeof id === 'string') deps.push({
                kind: 'row',
                model: relation.model.modelId,
                id
              });
            } else {
              const relatedRows = Array.isArray(resolved) ? resolved : resolved ? [resolved] : [];
              for (const relatedRow of relatedRows) deps.push({
                kind: 'row',
                model: relation.model.modelId,
                id: relatedRow.id
              });
            }
          } else {
            const idInclude = include;
            const [target, resolveIds, required] = Array.isArray(include) ? [include[0], include[1], undefined] : [idInclude.model, idInclude.ids, idInclude.require];
            const rawIds = resolveIds(row);
            const ids = idsOf(rawIds);
            const resolved = ids.map(id => target.find(id)).filter(candidate => (0, _requireFields.hasRequiredFields)(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys));
            included[alias] = Array.isArray(rawIds) ? resolved : resolved[0] ?? null;
            for (const id of ids) deps.push({
              kind: 'row',
              model: target.modelId,
              id
            });
          }
        }
        const current = cacheRef.current.get(row.id);
        if (current && current.row === row && (0, _rowEquality.rowsShallowEqual)(current.included, included)) return {
          row,
          included: current.included,
          item: current.item
        };
        const candidate = config.select ? config.select(row, included, {
          index
        }) : {
          ...row,
          ...included
        };
        const item = projectionGateRef.current.projectValue(row.id, candidate, candidate, config.renderKeys);
        cacheRef.current.set(row.id, {
          row,
          included,
          item
        });
        return {
          row,
          included,
          item
        };
      });
      for (const id of cacheRef.current.keys()) if (!liveIds.has(id)) cacheRef.current.delete(id);
      const resolved = scopeValue == null || sourceInternal.isResolved(scopeValue);
      if (!derivesOwnSet) return {
        items: projected.map(pair => pair.item),
        totalCount: rows.length,
        resolved,
        deps
      };
      let pairs = projected;
      if (config.filter) pairs = pairs.filter(pair => config.filter(pair.row, pair.included));
      if (config.sort) {
        const compare = (0, _ordering.compareRowsBySpec)(config.sort);
        const decorated = pairs.map(pair => ({
          pair,
          sortable: {
            ...pair.item,
            id: pair.row.id
          }
        }));
        decorated.sort((left, right) => compare(left.sortable, right.sortable));
        pairs = decorated.map(entry => entry.pair);
      }
      const windowed = limit === null ? pairs : pairs.slice(0, limit);
      return {
        items: windowed.map(pair => pair.item),
        totalCount: pairs.length,
        resolved,
        deps
      };
    };
    if (evaluatedRef.current === null || evaluatedRef.current.scopeKey !== scopeKey || evaluatedRef.current.limit !== limit) {
      const result = evaluate();
      evaluatedRef.current = {
        scopeKey,
        limit,
        snapshot: {
          items: result.items,
          totalCount: result.totalCount,
          resolved: result.resolved
        },
        deps: result.deps
      };
    }
    return (0, _useLiveRead.useLiveRead)(() => {
      const result = evaluate();
      const snapshot = {
        items: result.items,
        totalCount: result.totalCount,
        resolved: result.resolved
      };
      evaluatedRef.current = {
        scopeKey,
        limit,
        snapshot,
        deps: result.deps
      };
      return snapshot;
    }, evaluatedRef.current.deps, (left, right) => left.resolved === right.resolved && left.totalCount === right.totalCount && (0, _arrayEquality.arraysShallowEqual)(left.items, right.items), (0, _serialize.compositeKey)(scopeKey ?? '', limit === null ? 'all' : String(limit)));
  };
  return {
    use: (scopeValue, options) => {
      const snapshot = useItems(scopeValue, null);
      const scopeKey = scopeValue == null ? null : sourceInternal.key(scopeValue);
      return (0, _scopeRetention.useScopeRetention)(scopeKey, {
        rows: snapshot.items,
        totalCount: snapshot.totalCount
      }, snapshot.resolved, options?.keepPrevious === true).snapshot.rows;
    },
    useWindow: (scopeValue, options) => {
      const pageSize = options?.pageSize ?? (0, _configure.getDbRuntimeConfig)().defaults?.pageSize ?? 20;
      const scopeKey = scopeValue == null ? null : sourceInternal.key(scopeValue);
      const [state, setState] = (0, _react.useState)({
        scopeKey,
        size: pageSize
      });
      const size = state.scopeKey === scopeKey ? state.size : pageSize;
      const windowRef = (0, _react.useRef)(null);
      const snapshot = useItems(scopeValue, size);
      const items = snapshot.items;
      const cached = windowRef.current;
      const rows = cached && cached.items === items && cached.size === size ? cached.rows : items.slice(0, size);
      windowRef.current = {
        items,
        size,
        rows
      };
      const retained = (0, _scopeRetention.useScopeRetention)(scopeKey, {
        rows,
        totalCount: snapshot.totalCount
      }, snapshot.resolved, options?.keepPrevious === true);
      return {
        rows: retained.snapshot.rows,
        totalCount: retained.snapshot.totalCount,
        hasMore: retained.snapshot.totalCount > size,
        isPreviousData: retained.isPreviousData,
        resolved: snapshot.resolved,
        fetchNextPage: () => setState(current => current.scopeKey === scopeKey ? {
          ...current,
          size: current.size + pageSize
        } : {
          scopeKey,
          size: pageSize + pageSize
        })
      };
    }
  };
};
exports.defineView = defineView;
//# sourceMappingURL=defineView.js.map