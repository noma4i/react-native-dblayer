      ...
      const items = visibleRows.map((row, index) => {
        ...
        for (const [alias, include] of Object.entries(config.include)) {
          ...
          if (relationName) {
            ...
            included[alias] = Array.isArray(resolved) ? resolved.filter(candidate => (0, _requireFields.hasRequiredFields)(candidate, required ?? [])) : (0, _requireFields.hasRequiredFields)(resolved, required ?? []) ? resolved : null;
            if (relation.kind === 'belongsTo') {
              const id = row[relation.foreignKey];
            ...
            const resolved = ids.map(id => target.get(id)).filter(candidate => (0, _requireFields.hasRequiredFields)(candidate, required ?? []));
