...
const foreignKeyIndexFor = (target, foreignKey) => {
  ...
  index.unsubscribe = (0, _configure.getCommitBus)().subscribeAll(batch => {
    ...
    for (const change of batch.rows) {
      ...
      const next = target.get(change.id);
      ...
      const items = visibleRows.map((row, index) => {
        ...
        for (const [alias, include] of Object.entries(config.include)) {
          ...
          if (relationName) {
            ...
            const resolved = ids.map(id => target.get(id)).filter(candidate => (0, _requireFields.hasRequiredFields)(candidate, required ?? [])).map(candidate => projectIncludedRow(alias, candidate, includeRenderKeys));
