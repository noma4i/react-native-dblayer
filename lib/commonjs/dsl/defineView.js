...
const defineView = (model, name, publicConfig) => {
  ...
  const useItems = (scopeValue, limit) => {
    ...
    return (0, _useLiveRead.useLiveRead)(() => {
      ...
      return snapshot;
    }, evaluatedRef.current.deps, (left, right) => left.resolved === right.resolved && left.totalCount === right.totalCount && (0, _useLiveRead.arraysShallowEqual)(left.items, right.items));
  };
  return {
