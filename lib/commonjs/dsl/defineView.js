...
const includesEqual = (left, right) => {
  ...
  return keys.length === Object.keys(right).length && keys.every(key => Array.isArray(left[key]) && Array.isArray(right[key]) ? (0, _useLiveRead.arraysShallowEqual)(left[key], right[key]) : Object.is(left[key], right[key]));
};
...
const defineView = (model, name, publicConfig) => {
  ...
  const useItems = (scopeValue, limit) => {
    ...
    const relationIndexesRef = (0, _react.useRef)(new Map());
    ...
    const rowsFor = (relation, foreignKey, id) => {
      ...
      const revision = (0, _internalHandles.getInternalModelHandle)(target).revision();
    ...
    }, initial.deps, (left, right) => left.resolved === right.resolved && left.totalCount === right.totalCount && (0, _useLiveRead.arraysShallowEqual)(left.items, right.items));
  };
  return {
