...
const createModelContext = options => {
  ...
  const revisions = {
    admitRow: (incoming, previous, baseRevision) => {
      ...
      if (evictedFields > 0) (0, _diagnostics.noteCausalAdmissionDrop)();
      return admitted;
    },
    admitPatch: (id, patch, remove, previous, baseRevision) => {
      ...
      if (Object.keys(admittedPatch).length < Object.keys(patch).length || admittedRemove.length < remove.length) (0, _diagnostics.noteCausalAdmissionDrop)();
