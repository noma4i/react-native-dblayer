...
const createModelReadAccess = options => {
  ...
  const scopeSortedRows = (scopeName, scopeValue) => {
    ...
    return rows.sort((0, _ordering.compareRowsBySpec)(sort));
  };
  const whereRead = where => {
    ...
    return (0, _readBuilder.createReadBuilder)(where, {
      rows: function useRows(criteria, orders, limit, required, projection) {
        ...
        const signature = (0, _readIdentity.incrementalSignature)('where-builder', options.modelId, (0, _compileDbWhere.buildScopeKey)({
          criteria,
      ...
      pluck: function usePluck(criteria, orders, limit, required, projection, field) {
        ...
        const projectionRef = (0, _react.useRef)(projection);
        projectionRef.current = projection;
        const signature = (0, _readIdentity.incrementalSignature)('where-pluck', options.modelId, (0, _compileDbWhere.buildScopeKey)({
          criteria,
          orders: effectiveOrders,
          limit,
          required,
      ...
      exists: function useExists(criteria, required) {
        const signature = (0, _readIdentity.incrementalSignature)('where-exists', options.modelId, (0, _compileDbWhere.buildScopeKey)({
          criteria,
          required
        }));
