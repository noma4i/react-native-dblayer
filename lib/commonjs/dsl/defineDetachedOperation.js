...
const reportFailure = (error, model) => {
  try {
    (0, _configure.getDbRuntimeConfig)().defaults?.onSyncError?.(error, {
      source: 'detached',
      model
    });
  } catch {
...
const defineDetachedOperation = (model, kind, config) => {
  ...
  const failRecord = (record, error) => {
    ...
    if (config.failure === 'rollback') {
      (0, _configure.getOperationState)().close(record.operationId, 'rolledback', {
        persist: false
      });
      (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)([{
        ...
        ids: record.tempIds,
        tombstone: false
      }], () => (0, _configure.getOperationState)().persistEntries()));
  ...
  const resumeRecord = async record => {
  ...
  const handle = {
    start: input => {
      ...
      (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(ops, () => operations.persistEntries()));
      return {
        operationId,
        tempId
      };
    ...
    complete: (operationId, serverNode) => {
      ...
      (0, _configure.getOperationState)().close(operationId, 'committed', {
        persist: false
      });
      (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(ops, () => (0, _configure.getOperationState)().persistEntries()));
    },
  ...
  declarations.set(kind, {
    generation,
    resume: async record => void (await resumeRecord(record))
  });
  return handle;
};
...
const reconcileDetachedOperationsAtBoot = async () => {
