...
exports.defineMutation = exports.clearFailedOptimisticMutation = void 0;
...
const defineMutation = config => {
  ...
  const planFromRespond = (data, context, optimistic, input) => {
    ...
    if (node != null) {
      ...
      if (context.tempId && id !== context.tempId && optimistic.model.find(context.tempId) !== undefined) ops.push(...(0, _internalHandles.getInternalModelHandle)(optimistic.model).planReplace(context.tempId, row));else ops.push(...(0, _internalHandles.getInternalModelHandle)(optimistic.model).planRows([row]));
  ...
  const runWithTempId = async (input, forcedTempId) => {
    ...
    try {
      ...
      if (optimistic && isRespondOptimistic(optimistic)) {
        ...
        if (optimisticOps.length > 0) (0, _configure.getApplyRuntime)().apply(optimisticOps);
        ...
        if (reuseId != null && (forcedTempId != null || optimistic.model.find(reuseId) !== undefined)) {
          ...
          if (placement) {
            ops.push(...(0, _internalHandles.getInternalScopeHandle)(placement.scope).planPlacement(placement.value(input), newTempId, position));
          }
        ...
        (0, _internalHandles.getInternalModelHandle)(optimistic.model).applyPatch(String(id), optimistic.selectPatch(input), operationId);
      ...
      if (optimistic && isMethodOptimistic(optimistic) && optimistic.method === 'patch' && (0, _normalizeHelpers.isRecord)(previous)) {
        ...
        if (Object.keys(restore).length > 0) (0, _internalHandles.getInternalModelHandle)(optimistic.model).applyPatch(String(optimistic.selectId(input)), restore, operationId);
      }
      if (optimistic && isMethodOptimistic(optimistic) && optimistic.method === 'destroy' && (0, _normalizeHelpers.isRecord)(previous)) {
        (0, _configure.getApplyRuntime)().apply((0, _internalHandles.getInternalModelHandle)(optimistic.model).planRestore(previous, previousMemberships));
      }
