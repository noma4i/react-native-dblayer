...
const defineMutation = config => {
  ...
  const dedupePlacementAppends = (plan, placementOps) => {
    ...
    for (const op of placementOps) {
      ...
      const key = (0, _serialize.compositeKey)(op.model, op.scopeKey);
    ...
    return plan.flatMap(op => {
      ...
      const ids = placementIds.get((0, _serialize.compositeKey)(op.model, op.scopeKey));
      if (!ids) return [op];
    ...
    try {
      ...
      if (optimistic && isRespondOptimistic(optimistic) && insertedTempId) {
        ...
        if (optimistic.failure === 'rollback') {
          if (insertedTempId) (0, _configure.getApplyRuntime)().apply((0, _relations.expandPlan)([{
            kind: 'destroy',
            model: optimistic.model.modelId,
            ids: [insertedTempId],
            tombstone: false
