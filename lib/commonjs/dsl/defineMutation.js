...
const defineMutation = config => {
  ...
  const planFromRespond = (data, context, optimistic, input) => {
    ...
    if (node != null) {
      ...
      if (context.tempId && id !== context.tempId && optimistic.model.get(context.tempId) !== undefined) ops.push(...(optimistic.model.__planReplace?.(context.tempId, row) ?? []));
        ...
        ops.push(
          ...(optimistic.model.__planRows?.([row]) ?? [
            {
              kind: 'upsert',
  ...
  const inverseFromRespond = (data, context, optimistic) => {
    ...
    for (const sink of config.extract?.({
      ...
      for (const row of sink.rows)
        if ((0, _normalizeHelpers.isRecord)(row) && typeof row.id === 'string')
          targets.push({
            model,
            id: row.id
          });
    ...
    return targets.flatMap(({ model, id }) => {
      ...
      return (
        model.__planRestore?.(previous, memberships) ?? [
          {
            kind: 'upsert',
            model: model.modelId,
            rows: [previous],
  ...
  return {
    ...
    use: () => {
      ...
      const mutate = (0, _react.useCallback)(
        (input, callbacks) => {
          mutateAsync(input)
