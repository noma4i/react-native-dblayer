...
const createModelDirectAccess = options => {
  ...
  return {
    ...
    where: (where, readOptions) => {
      ...
      if (!order) return (0, _ordering.limitRows)(rows, readOptions?.limit);
      return (0, _ordering.sortModelReadRows)(rows, [{
    ...
    invalidate: scope => {
      const covered = (0, _invalidationRegistry.invalidateModel)(options.modelId, scope);
