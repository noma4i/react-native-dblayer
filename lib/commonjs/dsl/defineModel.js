...
const defineModel = config => {
  ...
  function whereRead(where) {
    return (0, _readBuilder.createReadBuilder)(where, {
      ...
      read: (criteria, orders, limit, required, projection) => {
        const rows = planes().entityState.values().filter(row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria) && (0, _requireFields.hasRequiredFields)(row, required));
  ...
  const makeScopeHandle = scopeName => {
      ...
      if (maxRows != null && (opts?.resetOrder === true || coverage === 'complete') && next.entries.length > maxRows) {
        if (spec.sort && spec.sort !== 'server-order') {
          ...
          if ('comparator' in scopeSort) {
            ...
            const ordered = (0, _incrementalReadEngine.sortModelReadRows)([...rowsById.values()], [{
    ...
    return {
      ...
      __isResolved: scopeValue => planes().scopeIndex.read(keyForScope(scopeName, scopeValue)).generation > 0,
  ...
  const model = {
    ...
    getWhere: (where, options) => {
      const rows = planes().entityState.values().filter(row => (0, _compileDbWhere.matchesDbWhere)(row, where));
    ...
    use: {
      pending: id => {
        const readPending = (0, _react.useCallback)(() => id != null && (0, _configure.getOperationState)().pending().some(operation => operation.model === config.id && (operation.rowIds ?? operation.tempIds).includes(id)), [id]);
      ...
      byIds: (ids, options = {}) => {
        ...
        const rows = (0, _projectionGate.useProjectedLiveRows)(() => resolvedIds.map(id => planes().entityState.read(id)).filter(Boolean), resolvedIds.map(id => rowDep(id)), options, `${config.id}.use.byIds`);
