...
const keyForScope = scopeValue => (0, _compileDbWhere.buildScopeKey)(scopeValue);
const EMPTY_ROWS = [];
...
const defineModel = config => {
  ...
  const isScopeMember = (scopeKey, id) => planes().scopeIndex.read(scopeKey).entries.some(entry => entry.id === id);
  const membershipAppend = (scopeKey, id) => {
    const {
      next
    } = planes().scopeIndex.reconcile(scopeKey, 'delta', [{
      id
    }]);
    return {
      kind: 'scope',
  ...
  const membershipDetach = (scopeKey, id) => ({
    kind: 'scope',
    model: config.id,
    scopeKey,
    next: planes().scopeIndex.detach(scopeKey, [id])
  });
  ...
  const makeScopeHandle = scopeName => {
    const planApply = (scopeValue, rows, coverage) => {
      ...
      const {
        next
      } = planes().scopeIndex.reconcile(scopeKey, coverage, liveRows.map(({
        row,
        edge
      }) => ({
        id: row.id,
    ...
    return {
      use: scopeValue => {
        const rows = (0, _useLiveRead.useLiveRead)(() => scopeValue == null ? EMPTY_ROWS : scopeSortedRows(scopeName, scopeValue), scopeValue == null ? [modelDep] : memberDeps(keyForScope(scopeValue), planes().scopeIndex.read(keyForScope(scopeValue)).entries), _useLiveRead.arraysShallowEqual);
        return rows;
      },
      ...
      useCount: scopeValue => (0, _useLiveRead.useLiveRead)(() => scopeValue == null ? 0 : planes().scopeIndex.read(keyForScope(scopeValue)).entries.length, scopeValue == null ? [modelDep] : [scopeDep(keyForScope(scopeValue))]),
  ...
  const model = {
    ...
    use: {
      ...
      first: (where, options) => (0, _useLiveRead.useLiveRead)(() => sortRows(planes().entityState.values().filter(row => where == null || (0, _compileDbWhere.matchesDbWhere)(row, where)), options)[0], [modelDep]),
      where: (where, options) => (0, _useLiveRead.useLiveRead)(() => where == null ? EMPTY_ROWS : sortRows(planes().entityState.values().filter(row => (0, _compileDbWhere.matchesDbWhere)(row, where)), options), where == null ? [] : [modelDep], _useLiveRead.arraysShallowEqual),
      ...
      count: where => (0, _useLiveRead.useLiveRead)(() => where == null ? planes().entityState.values().length : planes().entityState.values().filter(row => (0, _compileDbWhere.matchesDbWhere)(row, where)).length, [modelDep]),
