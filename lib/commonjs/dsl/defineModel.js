...
const defineModel = config => {
  ...
  const normalize = (input, complete = false) => {
    ...
    const id = config.rowId?.(input) ?? ((0, _normalizeHelpers.isRecord)(input) ? input.id : undefined);
  ...
  const membershipScopes = Object.entries(config.scopes ?? {}).flatMap(([name, spec]) =>
    spec.by
      ? [
          [
            name,
  ...
  const membershipForUpsert = row => {
    ...
    for (const [scopeName, spec] of membershipScopes) {
      ...
      if (beforeKey && beforeKey !== afterKey && isScopeMember(beforeKey, id))
        deltas.push({
          scopeKey: beforeKey,
          detach: [id]
        });
      if (afterKey && !isScopeMember(afterKey, id))
        deltas.push({
          scopeKey: afterKey,
          append: [id]
        });
  ...
  const applyEvent = ops => {
    (0, _configure.getApplyRuntime)().apply(
  ...
  function whereRead(where) {
    return (0, _readBuilder.createReadBuilder)(where, {
      rows: (criteria, orders, limit, required, projection) => {
        ...
        const signature = (0, _incrementalReadEngine.incrementalSignature)(
          'where-builder',
          config.id,
          (0, _compileDbWhere.buildScopeKey)({
            criteria,
            orders,
            limit,
            required
        ...
        return (0, _incrementalReadEngine.useIncrementalRead)({
          ...
          create: () =>
            (0, _incrementalReadEngine.createModelReadEngine)({
              signature,
              model: config.id,
              where: row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria) && (0, _requireFields.hasRequiredFields)(row, required),
              options: {
                orderBy: orders,
                limit
              },
      ...
      read: (criteria, orders, limit, required, projection) => {
        const rows = planes()
          .entityState.values()
          .filter(row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria) && (0, _requireFields.hasRequiredFields)(row, required));
        const selected = orders.length > 0 ? (0, _incrementalReadEngine.sortModelReadRows)(rows, orders, limit) : (0, _incrementalReadEngine.limitRows)(rows, limit);
  ...
  const makeScopeHandle = scopeName => {
      ...
      if (maxRows != null && (opts?.resetOrder === true || coverage === 'complete') && next.entries.length > maxRows) {
        if (spec.sort && spec.sort !== 'server-order') {
          ...
          if ('comparator' in scopeSort) {
            ...
            const ordered = (0, _incrementalReadEngine.sortModelReadRows)(
              [...rowsById.values()],
              [
                {
    ...
    const scopeHandle = {
      ...
      use: (scopeValue, options = {}) => {
        ...
        return (0, _liveScopeReads.useScopeLiveRows)(
          config.id,
          scopeKey,
      ...
      useWindow: (scopeValue, options = {}) => {
        ...
        if (windowState.scopeKey !== scopeKey)
          setWindowState({
            scopeKey,
            size: pageSize
          });
        useScopeAccess(scopeKey);
        const window = (0, _liveScopeReads.useScopeLiveWindowRows)(
          config.id,
          scopeKey,
          applyTarget.scopeSortMeta(scopeKey ?? `${scopeName}:`),
          windowSize,
  ...
  const restoreMembership = (nextId, memberships) =>
  ...
  const model = {
    ...
    mutation: (name, mutationConfig) => {
      const dedupe =
        mutationConfig.dedupe === false
          ? false
          : (mutationConfig.dedupe ?? {
    ...
    fetch: (name, fetchConfig) =>
      (0, _defineFetch.defineFetch)({
        ...fetchConfig,
        key: fetchConfig.key ?? `${config.id}:${name}`
      }),
    poller: (name, pollerConfig) =>
      (0, _modelStatusPoller.createModelStatusPoller)({
        ...pollerConfig,
        fetch: async id => {
          try {
            return (
              await (0, _transport.getDbTransport)().query({
    ...
    getWhere: (where, options) => {
      const rows = planes()
        .entityState.values()
        .filter(row => (0, _compileDbWhere.matchesDbWhere)(row, where));
      if (!options?.orderBy) return (0, _incrementalReadEngine.limitRows)(rows, options?.limit);
      return (0, _incrementalReadEngine.sortModelReadRows)(
        rows,
        [
          {
    ...
    use: {
      pending: id => {
        const readPending = (0, _react.useCallback)(
          () =>
            id != null &&
        ...
        const subscribePending = (0, _react.useCallback)(
          listener => {
            if (id == null) return () => {};
      ...
      first: (where, options = {}) => {
        ...
        const signature = (0, _incrementalReadEngine.incrementalSignature)('first', config.id, where, options.orderBy, options.limit, options.require);
        return (0, _incrementalReadEngine.useIncrementalRead)({
          signature,
          deps: [modelDep],
          create: () =>
            (0, _incrementalReadEngine.createModelReadEngine)({
              signature,
              model: config.id,
              where: row => (where == null || (0, _compileDbWhere.matchesDbWhere)(row, where)) && (0, _requireFields.hasRequiredFields)(row, optionsRef.current.require ?? []),
      ...
      byIds: (ids, options = {}) => {
        ...
        const rows = (0, _projectionGate.useProjectedLiveRows)(
          () => resolvedIds.map(id => planes().entityState.read(id)).filter(row => row !== undefined),
          resolvedIds.map(id => rowDep(id)),
          options,
          `${config.id}.use.byIds`
        );
        const resultRef = (0, _react.useRef)(null);
        if (resultRef.current?.rows !== rows)
          resultRef.current = {
            rows,
      ...
      count: where =>
        (0, _incrementalReadEngine.useIncrementalRead)({
      ...
      related: (id, relationName, options = {}) => {
        ...
        if (relation.kind === 'hasMany') {
          return (0, _projectionGate.useProjectedLiveRows)(
            () =>
              id == null
                ? EMPTY_ROWS
                : relation.model.getWhere({
