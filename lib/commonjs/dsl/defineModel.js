...
const defineModel = config => {
  ...
  function whereRead(where, options) {
    if (options) {
    ...
    return (0, _readBuilder.createReadBuilder)(where, {
      ...
      read: (criteria, orders, limit) => (0, _incrementalReadEngine.sortModelReadRows)(planes().entityState.values().filter(row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria)), orders, limit)
    });
  }
