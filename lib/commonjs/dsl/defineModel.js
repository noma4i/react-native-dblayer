...
const defineModel = config => {
  ...
  function whereRead(where) {
    return (0, _readBuilder.createReadBuilder)(where, {
      rows: (criteria, orders, limit, required) => (0, _incrementalReadEngine.useIncrementalRead)({
        signature: (0, _incrementalReadEngine.incrementalSignature)('where-builder', config.id, (0, _compileDbWhere.buildScopeKey)({
          criteria,
          orders,
          limit
        })),
      ...
      read: (criteria, orders, limit, required) => {
        ...
        if (orders.length > 0) return (0, _incrementalReadEngine.sortModelReadRows)(rows, orders, limit);
        return (0, _incrementalReadEngine.limitRows)(rows, limit);
      }
    });
  }
    ...
    use: {
      ...
      first: (where, options) => (0, _incrementalReadEngine.useIncrementalRead)({
        ...
        create: () => (0, _incrementalReadEngine.createModelReadEngine)({
          signature: (0, _incrementalReadEngine.incrementalSignature)('first', config.id, where, options),
          model: config.id,
          where: row => (where == null || (0, _compileDbWhere.matchesDbWhere)(row, where)) && (0, _requireFields.hasRequiredFields)(row, options?.require ?? []),
          options: options ? {
