...
var _recovery = require("../core/recovery.js");
...
const sortRowsBySpec = (rows, sort) => 'comparator' in sort ? [...rows].sort(sort.comparator) : (0, _incrementalReadEngine.sortModelReadRows)(rows, [{
...
const defineModel = config => {
  ...
  const planes = () => {
    ...
    const entityState = (0, _entityState.createEntityState)({
      ...
      applyWriteGate,
      ownedFields: (rowId, operationId) => (0, _configure.getOperationState)().ownedFields(config.id, rowId, operationId)
    });
    ...
    try {
      ...
      if (!(error instanceof _recovery.CorruptionError)) throw error;
      (0, _recovery.coldResetModel)(runtime.storage, (0, _configure.getStoragePrefix)(), config.id);
  ...
  const keyForScope = (scopeName, scopeValue) => {
    ...
    return `${scopeName}:${(0, _compileDbWhere.buildScopeKey)(coerceScopeValueForKey(scopeName, scopeValue))}`;
  };
  ...
  const applyEvent = ops => {
    (0, _configure.getApplyRuntime)().apply(ops.map(op => op.kind === 'upsert' && op.origin === undefined ? {
      kind: 'upsert',
      model: op.model,
      rows: op.rows,
      origin: 'event'
  ...
  const makeScopeHandle = scopeName => {
    ...
    const scopeHandle = {
      ...
      issueSequence: (scopeValue, field) => {
        ...
        const issuedKey = (0, _serialize.compositeKey)(config.id, scopeKey, field);
  ...
  const model = {
    ...
    use: {
      pending: id => {
        ...
        const readPending = (0, _react.useCallback)(() => key != null && (0, _configure.getOperationState)().pendingForRow(config.id, key).length > 0, [key]);
      ...
      unsyncedChanges: id => {
        ...
        const readChanges = (0, _react.useCallback)(() => {
          ...
          let merged;
          for (const operation of (0, _configure.getOperationState)().pendingForRow(config.id, key)) {
      ...
      byIds: (ids, options = {}) => {
        ...
        (0, _projectionGate.validateProjectionOptions)(options, `${config.id}.use.byIds`);
        const optionsRef = (0, _react.useRef)(options);
        const gateRef = (0, _react.useRef)((0, _projectionGate.createProjectionGate)());
  ...
  (0, _internalHandles.registerInternalModelHandle)(model, {
    ...
    applyPatch: (id, patch, operationId) => (0, _configure.getApplyRuntime)().apply([{
      kind: 'patch',
      model: config.id,
      id: String(id),
      patch,
