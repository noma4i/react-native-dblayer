  ...
  const makeScopeHandle = scopeName => {
    ...
    return {
      ...
      use: (scopeValue, options = {}) => {
        ...
        return (0, _liveScopeReads.useScopeLiveRows)(config.id, scopeKey, applyTarget.scopeSortMeta(scopeKey ?? `${scopeName}:`), options);
      },
      useWindow: (scopeValue, options = {}) => {
        ...
        useScopeAccess(scopeKey);
        const window = (0, _liveScopeReads.useScopeLiveWindowRows)(config.id, scopeKey, applyTarget.scopeSortMeta(scopeKey ?? `${scopeName}:`), windowSize, options);
        return {
          rows: window.rows,
