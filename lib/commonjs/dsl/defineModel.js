  ...
  const makeScopeHandle = scopeName => {
    ...
    return {
      ...
      use: scopeValue => {
        ...
        const signature = (0, _incrementalReadEngine.incrementalSignature)('scope', config.id, scopeName, scopeValue);
        return (0, _incrementalReadEngine.useIncrementalRead)({
          signature,
          deps: scopeKey == null ? [] : memberDeps(scopeKey),
          create: () =>
            (0, _incrementalReadEngine.createScopeReadEngine)({
              signature,
              model: config.id,
              scopeKey: scopeKey ?? '',
              ...
              sort:
                spec?.sort === 'server-order' || spec?.sort == null
                  ...
                  : 'comparator' in spec.sort
                    ? spec.sort
                    : {
                        field: String(spec.sort.field),
                        direction: spec.sort.dir
                      }
            })
        });
      },
      useWindow: (scopeValue, options) => {
        ...
        const signature = (0, _incrementalReadEngine.incrementalSignature)('scope-window', config.id, scopeName, scopeValue, windowSize);
        const deps = (0, _react.useMemo)(() => (scopeKey == null ? [] : memberDeps(scopeKey)), [scopeKey]);
        const rows = (0, _incrementalReadEngine.useIncrementalRead)({
          signature,
          deps,
          create: () =>
            (0, _incrementalReadEngine.createScopeReadEngine)({
    ...
    use: {
      ...
      byIds: ids =>
        (0, _useLiveRead.useLiveRead)(
          ...
          _useLiveRead.arraysShallowEqual
        ),
      count: where =>
        (0, _incrementalReadEngine.useIncrementalRead)({
