...
const defineModel = config => {
  ...
  const isScopeMember = (scopeKey, id) => scopeIndex.read(scopeKey).entries.some(entry => entry.id === id);
  const membershipAppend = (scopeKey, id) => {
    const {
      next
    } = scopeIndex.reconcile(scopeKey, 'delta', [{
      id
    }]);
    return {
      kind: 'scope',
  ...
  const membershipDetach = (scopeKey, id) => ({
    ...
    model: config.id,
    scopeKey,
    next: scopeIndex.detach(scopeKey, [id])
  });
