...
const defineModel = config => {
  ...
  function whereRead(where) {
    return (0, _readBuilder.createReadBuilder)(where, {
      rows: (criteria, orders, limit) => (0, _incrementalReadEngine.useIncrementalRead)({
        ...
        create: () => (0, _incrementalReadEngine.createModelReadEngine)({
          ...
          model: config.id,
          where: row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria),
          options: {
            orderBy: orders,
            limit
          },
      ...
      read: (criteria, orders, limit) => {
        const rows = planes().entityState.values().filter(row => criteria != null && (0, _compileDbWhere.matchesDbWhere)(row, criteria));
  ...
  const model = {
    ...
    poller: (_name, pollerConfig) => (0, _modelStatusPoller.createModelStatusPoller)({
      ...pollerConfig,
      fetch: async id => (await (0, _transport.getDbTransport)().query({
