...
const defineModel = config => {
  ...
  const makeScopeHandle = scopeName => {
    ...
    return {
      modelId: config.id,
      use: scopeValue => {
        const rows = (0, _useLiveRead.useLiveRead)(() => scopeValue == null ? EMPTY_ROWS : scopeSortedRows(scopeName, scopeValue), scopeValue == null ? [modelDep] : memberDeps(keyForScope(scopeName, scopeValue), planes().scopeIndex.read(keyForScope(scopeName, scopeValue)).entries), _useLiveRead.arraysShallowEqual);
        return rows;
      },
      useWindow: (scopeValue, options) => {
        ...
        return {
          ...
          loadMore: () => setWindowState(current => current.scopeKey === scopeKey ? {
            ...current,
