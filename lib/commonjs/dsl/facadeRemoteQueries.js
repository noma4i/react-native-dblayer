...
exports.compileRemoteRelations = void 0;
 ...
 */
const compileRemoteRelations = (runtime, relations) => {
  return Object.fromEntries(Object.entries(relations).map(([name, definition]) => {
    ...
    const refusedLanding = message => (0, _syncError.reportSyncError)(new Error(message), {
      source: 'query',
      model: runtime.modelId,
      key: name
    }, 'compileRemoteRelations');
    const query = remote ? remote.type === 'single' ? (() => {
      ...
      return runtime.query(name, {
        ...
        select: data => {
          ...
          return value.flatMap(node => node == null ? [] : [list.map ? list.map(node) : node]);
        },
        write: write ? ({
          data,
          nodes,
...
exports.compileRemoteRelations = compileRemoteRelations;
