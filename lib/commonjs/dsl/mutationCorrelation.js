...
exports.registerMutationCorrelator = exports.modelHasCorrelators = exports.correlateIncomingRow = void 0;
...
const registerMutationCorrelator = (modelId, mutationId, correlate) => {
...
const modelHasCorrelators = modelId => (correlators.get(modelId)?.size ?? 0) > 0;
...
const correlateIncomingRow = (modelId, incoming, options) => {
  ...
  for (const operation of (0, _configure.getOperationState)().openInsertsFor(modelId)) {
    const correlate = declared.get(operation.actionKey);
    if (!correlate) continue;
    ...
    if (!candidateMatches(correlate, candidate, incoming)) continue;
