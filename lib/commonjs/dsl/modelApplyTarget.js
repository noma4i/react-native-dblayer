...
const createModelApplyTarget = options => {
  ...
  const applyTarget = {
    ...
    planScopePlacement: (scopeKey, ids, readRow) => {
      ...
      const compare = (0, _modelReadAccess.compareRowsBySpec)(sort);
      const anchors = [];
      ...
      for (const id of ids) {
        ...
        if (!row) {
          placements.push({
            id,
            orderKey: (0, _orderKey.keysForSequence)(1, anchors.at(-1)?.orderKey)[0]
          });
          continue;
        }
        let lower = 0;
  ...
  (0, _transaction.registerApplyTarget)(options.modelId, applyTarget);
  const applySnapshot = ops => {
    (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(ops));
  };
  const applyEvent = ops => {
    (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)(ops.map(op => op.kind === 'upsert' && op.origin === undefined ? {
      kind: 'upsert',
      model: op.model,
      rows: op.rows,
      origin: 'event'
