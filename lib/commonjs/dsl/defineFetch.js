...
const defineFetch = config => {
  ...
  const execute = async (input, isCurrent) => {
    ...
    try {
      data = config.fetcher
        ? await config.fetcher(input)
        : (0, _transport.responseDataOrThrow)(
  ...
  const run = async (input, options) => {
    ...
    try {
      const data = await client.fetchQuery({
        queryKey,
        queryFn: async () => {
          const result = await execute(input, () => (0, _configure.getRuntimeGeneration)() === generation);
  ...
  const use = input => {
    ...
    (0, _react.useEffect)(() => {
      ...
      if (firstMount && canRefetch && !isFreshKey(key) && !state.isFetching)
        void run(input, {
          restart: false
        }).catch(() => {});
      const unsubscribeOnline = (0, _networkState.subscribeFetchNetwork)(() => {
        if ((0, _networkState.isFetchNetworkOnline)() && !isFreshKey(key))
          void run(input, {
            restart: false
          }).catch(() => {});
      });
