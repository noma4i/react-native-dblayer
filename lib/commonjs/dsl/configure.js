...
exports.resetPersistenceRuntime = exports.replayJournal = exports.purgeForeignStorageKeys = exports.noteMaintenancePersistence = exports.isDbConfigured = exports.getStoragePrefix = exports.getRuntimeGeneration = exports.getPersistenceDataVersion = exports.getOperationState = exports.getDbRuntimeConfig = exports.getDbQueryClient = exports.getCommitBus = exports.getApplyRuntime = exports.flushPersistence = exports.configureDb = exports.advanceRuntimeGeneration = void 0;
...
const configureDb = options => {
  ...
  stopMaintenanceScheduler = defaults.inSessionGc === false ? null : (0, _maintenanceScheduler.startMaintenanceScheduler)(defaults.inSessionGc);
  ...
  queryClient = new _reactQuery.QueryClient({
    defaultOptions: {
      queries: {
        ...
        retry: (failureCount, error) => (0, _networkState.isFetchNetworkOnline)() && (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount + 1) !== null,
        retryDelay: (failureCount, error) => (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount) ?? 0
      }
    }
  });
  return queryClient;
...
const replayJournal = () => {
  ...
  if (orphaned.length > 0) {
    ...
    runtime.commit((0, _transaction.createCommitEnvelope)(orphanDestroyOps, () => operations.persistEntries()));
  }
  ...
  for (const key of storage.keys(`${getStoragePrefix()}journal:`)) {
    ...
    for (const operation of record?.ops ?? []) {
      ...
      for (const row of operation.rows) noteCandidate(operation.model, typeof row === 'object' && row !== null ? row.id : undefined);
    }
  }
  const pendingTempIds = new Set(operations.pending().flatMap(operation => operation.tempIds));
  for (const [model, ids] of candidates) {
    const orphanIds = [...ids].filter(id => !pendingTempIds.has(id) && !operations.failedFor(model, id) && !(0, _maintenanceRegistry.isTempRowProtectedByModel)(model, id));
    if (orphanIds.length > 0 && hasApplyTarget(model)) runtime.commit((0, _transaction.createCommitEnvelope)([{
      kind: 'destroy',
      model,
      ids: orphanIds,
      tombstone: false
