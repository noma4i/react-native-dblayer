  ...
  queryClient = new _reactQuery.QueryClient({
    defaultOptions: {
      queries: {
        ...
        retry: (failureCount, error) => (0, _runtimeGeneration.getRuntimeGeneration)() === generation && (0, _networkState.isFetchNetworkOnline)() && (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount + 1) !== null,
...
const replayJournal = () => {
  ...
  if (orphaned.length > 0) {
    ...
    runtime.commit((0, _commitEnvelope.createCommitEnvelope)(orphanDestroyOps, orphanTransitions));
  }
