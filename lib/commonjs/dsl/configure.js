  ...
  queryClient = new _reactQuery.QueryClient({
    defaultOptions: {
      queries: {
        ...
        retry: (failureCount, error) => getRuntimeGeneration() === generation && (0, _networkState.isFetchNetworkOnline)() && (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount + 1) !== null,
        retryDelay: (failureCount, error) => getRuntimeGeneration() === generation ? (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount) ?? 0 : 0
      }
    }
  });
  return queryClient;
