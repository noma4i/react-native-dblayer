...
exports.resetPersistenceRuntime =
  ...
  exports.getPersistenceDataVersion =
  exports.getOperationState =
  exports.getDbRuntimeConfig =
  exports.getDbQueryClient =
    ...
    defaultOptions: {
      queries: {
        ...
        retry: (failureCount, error) =>
          ...
          (0, _networkState.isFetchNetworkOnline)() &&
          (0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount + 1) !== null,
        retryDelay: (failureCount, error) =>
          getRuntimeGeneration() === generation ? ((0, _retryPolicy.retryDelayMs)(getDbRuntimeConfig().defaults.retry?.query ?? {}, error, failureCount) ?? 0) : 0
      }
    }
  });
  return queryClient;
...
const replayJournal = () => {
  ...
  for (const [model, ids] of candidates) {
    const orphanIds = [...ids].filter(
      id => !openTempIds.has((0, _serialize.compositeKey)(model, id)) && !operations.failedFor(model, id) && !(0, _maintenanceRegistry.isTempRowProtectedByModel)(model, id)
    );
