...
var _quarantine = require("../core/quarantine.js");
var _storeUpsertResolver = require("../core/storeUpsertResolver.js");
...
const createModelWrites = options => {
  ...
  const preparePatch = (id, patch, previous, operationId, remove = [], baseRevision) => {
    ...
    return {
      row,
      changedFields: (0, _storeUpsertResolver.diffTopLevelFields)(previous, row)
    };
  };
  const putRows = rows => {
    const changes = [];
  ...
  const planReplace = (oldId, next, correlatedOperation) => {
    ...
    try {
      ...
      (0, _quarantine.putQuarantine)({
        kind: 'row',
        model: options.modelId,
        id: (0, _normalizeHelpers.isRecord)(next) && next.id !== undefined ? String(next.id) : '',
        raw: next,
    ...
    return [{
      ...
      origin: 'replace',
      mergeBase
    }, destroyLeg, ...restoreMembership(normalized.id, memberships)];
  };
  /**
