...
const createModelWrites = options => {
  const writeRows = (rows, origin, mergeBase, operationId) => {
    ...
    for (const value of rows) {
      ...
      try {
        ...
        (0, _logger.getDbLogger)().error(`[${options.modelName}] apply row rejected`, {
          error
        });
        continue;
      }
      if (origin === undefined && options.entityState().isTombstoned(incoming.id)) continue;
      const result = options.entityState().upsert(incoming, {
  ...
  const patchRow = (id, patch, operationId) => {
    const key = String(id);
    if (!options.entityState().read(key)) return null;
    const result = options.entityState().upsert({
      ...patch,
      id: key
    }, {
      ctx: {
  ...
  const splitCorrelatedRows = accepted => {
    ...
    for (const value of accepted) {
      ...
      const correlation = normalized ? (0, _mutationCorrelation.correlateIncomingRow)(options.modelId, normalized, {
