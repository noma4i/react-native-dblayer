...
const createNamedRelation = (runtime, name, params, query) => {
...
const createAction = (runtime, name, definition) => {
  ...
  const optimistic = definition.optimistic ? {
    ...
    build: (input, context) => {
      ...
      return definition.optimistic.build(input, {
        ...context,
...
const defineModelFacade = (key, config) => {
  ...
  const compiledRelations = Object.fromEntries(Object.entries(config.relations ?? {}).map(([name, definition]) => {
    ...
    const query = remote ? runtime.query(name, {
  ...
  for (const name of Object.keys(config.relations ?? {})) {
    Reflect.set(relationMethods, name, params => createNamedRelation(runtime, name, params, compiledRelations[name]));
  }
  ...
  const model = Object.assign(base, relationMethods);
