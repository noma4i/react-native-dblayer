...
const defineModelFacade = (key, config) => {
  ...
  const compiledRelations = (0, _facadeRemoteQueries.compileRemoteRelations)(runtime, config.relations);
  const relationMethods = Object.create(null);
  for (const name of Object.keys(config.relations ?? {})) {
    const method = params => (0, _facadeRelations.createNamedRelation)(runtime, name, params, compiledRelations[name], config.relations?.[name]?.remote?.type);
