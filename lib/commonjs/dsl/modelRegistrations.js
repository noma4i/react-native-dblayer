...
exports.registerModelSchemaAndGc = exports.registerModelRuntime = void 0;
...
const registerModelSchemaAndGc = options => {
  const {
    planes,
    resolvedRelations
  } = options.context;
  ...
  (0, _gc.registerGcHost)(options.modelId, {
...
exports.registerModelSchemaAndGc = registerModelSchemaAndGc;
const registerModelRuntime = options => {
  ...
  (0, _internalHandles.registerInternalModelHandle)(model, {
    ...
    applyPatch: (id, patch, operationId) => (0, _configure.getApplyRuntime)().commit((0, _transaction.createCommitEnvelope)([{
      kind: 'patch',
  ...
  if (options.maintenance) {
    const pendingTempRows = () => {
      ...
      const protectedIds = new Set([...(0, _configure.getOperationState)().pending().filter(operation => operation.model === options.modelId).flatMap(operation => operation.tempIds), ...modelProtectedTempIds()]);
      const ids = [];
