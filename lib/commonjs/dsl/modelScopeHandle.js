  ...
  return scopeName => {
    ...
    const planApply = (scopeValue, rows, coverage, planOptions) => {
      const liveRows = rows.filter(({
        ...
        row
      }) => !planes().entityState.isTombstoned(options.normalize(row).id));
    ...
    const scopeHandle = {
      ...
      useCount: scopeValue => {
        const scopeKey = scopeValue === null ? null : options.keyForScope(scopeName, scopeValue);
        options.useScopeAccess(scopeKey);
        return (0, _scopeReadEngine.useScopeReadCount)(options.modelId, scopeKey, options.applyTarget.scopeSortMeta(scopeKey ?? (0, _serialize.compositeKey)(scopeName, '')));
      },
      ...
      seed: (scopeValue, rows) => {
        const liveRows = rows.filter(options.isPlanRow).filter(row => !planes().entityState.isTombstoned(options.normalize(row).id)).map(row => ({
          row: row
        }));
