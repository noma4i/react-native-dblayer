...
const mergeMutationContexts = (optimisticContext, manualContext) => {
  ...
  if ((0, _normalizeHelpers.isRecord)(manualContext)) return {
    ...manualContext,
    ...optimisticContext
  };
...
const runDeclarativeOptimisticMutation = (optimistic, input) => {
  const existingTempId = optimistic.selectTempId ? optimistic.selectTempId(input) : (0, _executeDbMutation.readMutationTempId)(input);
  if (existingTempId) {
    return {
...
const useDbMutation = config => (0, _reactQuery.useMutation)({
  ...
  mutationFn: input => {
    ...
    const executeMutation = async () => {
      ...
      return (0, _executeDbMutation.executeDbMutationLifecycle)(config, input, mappedInput, {
        context: undefined,
        beforeRequest: () => {
          let context;
          if (shouldRunOptimisticMutation(config)) {
            tx.mutate(() => {
              context = (0, _registry.runInManagedMutationBatch)(() => runOptimisticMutation(config, input));
            });
          }
          return context;
        },
        commit: async (result, context) => {
          ...
          await tx.commit();
          if (applyError) {
            (0, _logger.getDbLogger)().error(logPrefix, 'commit-phase apply failed; optimistic state kept', applyError);
            throw applyError;
          }
        },
        rollback: () => {
          ...
          // promise rejection. The original error is still rethrown by the shared lifecycle.
    ...
    return (0, _singleFlight.runSingleFlight)((0, _singleFlight.createSingleFlightSignature)('db-mutation', mutationKey, dedupeKey), executeMutation);
  },
  onError: error => {
