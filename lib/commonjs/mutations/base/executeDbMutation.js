...
exports.runDbMutationDirect = exports.readMutationTempId = exports.executeDbMutationRequest = exports.executeDbMutationLifecycle = exports.applyDbMutationCommit = void 0;
...
const readMutationTempId = input => {
  if (!(0, _normalizeHelpers.isRecord)(input)) return null;
  const tempId = input.tempId;
  return typeof tempId === 'string' && tempId.length > 0 ? tempId : null;
};
exports.readMutationTempId = readMutationTempId;
...
const executeDbMutationLifecycle = async (config, input, mappedInput, options) => {
  ...
  try {
    ...
    (0, _mutationTracking.emitMutationTrackError)(config, mutationError, input);
    throw error;
  }
 ...
 * @returns Mutation result field or null.
 */
exports.executeDbMutationLifecycle = executeDbMutationLifecycle;
