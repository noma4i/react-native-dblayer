...
exports.trimRowsPerScope = exports.singletonStatics = exports.resolveStaleTempRows = exports.reconcileOptimisticRows = exports.pruneOrphanedRows = exports.pruneExpiredRows = exports.createThrottledSingleFlight = exports.createNestedObjectPatcher = exports.createKeyedBatchBuffer = exports.createKeyedArrayPatcher = exports.createIdArrayPatcher = void 0;
...
const destroyManyIfNeeded = (model, ids) => {
 ...
 */
const createKeyedBatchBuffer = config => {
  const buckets = new Map();
  let generation = null;
  const isCurrentGeneration = () => generation == null || generation === (0, _configure.getRuntimeGeneration)();
  ...
  const flushBucket = (key, bucket) => {
    ...
    try {
      ...
      (0, _logger.getDbLogger)().error('db', 'keyed batch buffer flush failed', {
        key,
        error
      });
    }
  };
  const scheduleBucket = (key, bucket) => {
    clearBucketTimer(bucket);
    bucket.timer = setTimeout(() => {
      flushBucket(key, bucket);
    }, config.flushMs);
  };
  const pushDistinct = (bucket, item) => {
    if (!config.dedupe) {
...
exports.createKeyedBatchBuffer = createKeyedBatchBuffer;
/**
...
const singletonStatics = (model, recordId, defaults) => {
  ...
  return {
    ...
    useCurrent: () => model.use.row(recordId) ?? defaults,
