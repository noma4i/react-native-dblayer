...
| `sort`      | `{ field, dir } \| { comparator } \| 'server-order'` | Member ordering: sort by a scalar stored field (`asc`/`desc`), a custom row comparator for composite values, or `'server-order'` (default). Missing values sort last; field and comparator ties use the codepoint `id` tie-break.                                                                                                                                                                                                                       |
