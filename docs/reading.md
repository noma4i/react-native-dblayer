...
A scope is a named, ordered subset of a model's rows, declared as a `ScopeSpec` literal (see
...
| `filter`               | `(row, included) => boolean`                        | Keep only accepted rows - the predicate sees the source row AND its resolved includes, so a view can filter by joined data a scope `member` predicate cannot reach (a scope `member` stays row-local by design). Applied before `sort` and the window; `totalCount` reflects the filtered set.                               |
| `sort`                 | `{ field, dir } \| Array<{ field, dir }> \| { comparator }` | Declared order of the projected ITEMS (same vocabulary as scope `sort`, id tie-break from the source row). Overrides the source scope order for this view's reads - e.g. an unread-first chat list over a recency-sorted scope.                                                                                       |
