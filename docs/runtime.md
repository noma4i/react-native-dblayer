...
| `readId`           | `(value: unknown) => string \| undefined`        | Canonical id coercion - the exact rule `f.id()` applies during `build`/`normalize`. Accepts strings and numbers, rejects everything else and the empty string, never trims. Use it on ingest/subscription payload ids before comparing them with stored rows or building scope values, so a numeric GraphQL id never misses a string bucket.                                                                                                   |
...
import { setFetchNetworkOnline } from '@noma4i/react-native-dblayer';
