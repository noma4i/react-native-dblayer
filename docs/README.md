...
- [Models](./models.md) - `defineModel` end-to-end: the `f`/`defineShape` field DSL, writes, reads,
  scopes, relations, and the reactivity guarantees every read relies on.
- [Queries](./queries.md) - `defineQuery` (network reads into a model/scope, pagination, coverage
  semantics), `defineFetch` (ephemeral store-free reads), and the stable view/list read helpers.
- [Mutations](./mutations.md) - `defineMutation` (optimistic lifecycle, temp-id replace, rollback,
  the cascade-destroy guard), `mergeOptimisticSnapshot`, and mutation error policy.
- [Configuration](./configuration.md) - `configureDb`/`DbDefaults`, `bootDb`/`suspendDb`, the
  composable startup/teardown primitives, `resetRuntime`, the storage/transport seams, the
  subscription runtime plus `defineIngest`, the persistence model, and the React Query passthrough.
...
Where a symbol is used from another doc's example (e.g. `belongsTo` inside a `defineQuery` extract
...
| Export | Kind | Home |
| --- | --- | --- |
| `defineQuery` | value | [queries.md](./queries.md#definequeryconfig) |
| `QueryResult` | type | [queries.md](./queries.md#queryresult) |
| `ExtractSink` | type | [queries.md](./queries.md#definequeryconfig) |
...
| Export | Kind | Home |
| --- | --- | --- |
| `defineMutation` | value | [mutations.md](./mutations.md#definemutationconfig) |
...
| Export | Kind | Home |
| --- | --- | --- |
| `defineIngest` | value | [configuration.md](./configuration.md#defineingestmodel-handlers) |
| `IngestDecl` | type | [configuration.md](./configuration.md#defineingestmodel-handlers) |
