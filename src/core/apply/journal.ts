...
export const createJournal = (storage: StoragePlane, prefix: () => string) => {
  ...
  return {
    ...
    committedEntry: (record: JournalRecord): Array<{ key: string; value: string | null }> => {
      const entries: Array<{ key: string; value: string | null }> = [
        { key: recordKey(record.epoch), value: JSON.stringify({ ...record, status: 'committed' }) }
      ];
      const committed = allRecords().filter(other => other.status === 'committed' && other.epoch !== record.epoch);
