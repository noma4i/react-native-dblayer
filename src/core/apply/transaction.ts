...
import { deltaKey, encodeDelta, highestPersistedSeq, readDeltaLog, readSnapseq, snapseqKey } from './deltaLog';
...
export const createApplyRuntime = (options: { storage: StoragePlane; prefix: () => string; bus: CommitBus }): ApplyRuntime => {
  ...
  const pendingDeltaModels = new Map<number, string[]>();
  const snapseqCache = new Map<string, number>();
  let flushTimer: ReturnType<typeof setTimeout> | null = null;

  const snapseqOf = (model: string): number => {
  ...
  const persistCommit = (ops: AppliedOp[], transitions: readonly OperationTransition[]): void => {
    ...
    if (ops.length > 0) {
      ...
      try {
        ...
        deltaSeq = seq;
        getDbLogger().error('commit delta write refused, snapshot flush will cover', { seq, error });
  ...
  const applyWithRecovery = (ops: AppliedOp[], commitEpoch: number, persist: () => void): IncrementalCommitBatch => {
