...
export const createKeyedLocalState = <TState extends Record<string, unknown>>(initial: TState): KeyedLocalState<TState> => {
  ...
  return {
    ...
    set: (key, next) => {
      ...
      if ((Object.keys(merged) as Array<keyof TState>).every(field => merged[field] === previous[field])) return;
