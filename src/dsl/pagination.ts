 ...
 */
export const bridgeWindowPagination = <T>(
  window: ScopeWindowResult<T>,
  query: Pick<QueryResult<unknown>, 'hasNextPage' | 'isFetchingNextPage' | 'fetchNextPage' | 'loadingState' | 'error'>
): WindowPaginationBridge<T> => ({
  rows: window.rows,
  ...
  fetchNextPage: () => {
    if (window.hasMore) window.fetchNextPage();
