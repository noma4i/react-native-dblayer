...
export const createModelDirectAccess = <TStored extends { id: string; updatedAt?: string | null } & Record<string, unknown>, TInput>(
...
): ModelDirectAccess<TStored, TInput> => {
  ...
  return {
    ...
    where: (where, readOptions) => {
      const rows = planes()
        .entityState.values()
        .filter(row => options.matchesCriteria(row, where));
      const order = readOptions?.orderBy ?? options.defaultOrder;
      if (!order) return limitRows(rows, readOptions?.limit);
      return sortModelReadRows(rows, [{ field: String(order.field), direction: order.direction }], readOptions?.limit);
    },
    all: () => planes().entityState.values(),
    update: (id, patch) => options.applyEvent([{ kind: 'patch', model: options.modelId, id: String(id), patch: patch as Record<string, unknown> }]),
    destroy: id => options.applyEvent([{ kind: 'destroy', model: options.modelId, ids: [String(id)] }]),
    destroyMany: ids => options.applyEvent([{ kind: 'destroy', model: options.modelId, ids: ids.map(id => String(id)) }]),
    updateAll: (where, patch) => {
      const rows = planes()
        .entityState.values()
        .filter(row => options.matchesCriteria(row, where));
      if (rows.length === 0) return 0;
      options.applyEvent(rows.map(row => ({ kind: 'patch', model: options.modelId, id: String(row.id), patch: patch as Record<string, unknown> })));
      return rows.length;
    },
    destroyAll: where => {
      const ids = planes()
