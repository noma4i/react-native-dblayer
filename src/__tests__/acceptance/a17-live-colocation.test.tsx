...
describe('A17 live colocation', () => {
  ...
  it('stops unmounted live runtime and reactivates mounted readers across reset', () => {
    const subscribers: Array<{ next(data: unknown): void }> = []; const { queryClient } = setupAcceptanceRuntime({ transport: createAcceptanceTransport({ subscribe: (_options, handlers) => { subscribers.push(handlers); return () => {}; } }) }); const model = make('A17Reset'); const list = model.query('list', { document, select: () => [], into: model.scopes.feed, live: { created: { document, handler: payload => ({ upsert: payload }) } } }); const wrap = (child: React.ReactElement) => React.createElement(QueryClientProvider, { client: queryClient }, child); const reader = renderCounted(() => list.use(value).data, wrap); reader.unmount(); resetRuntime(); subscribers[0]?.next({ created: { id: 'late', group: 'g', title: 'late' } }); expect(model.get('late')).toBeUndefined(); const mounted = renderCounted(() => list.use(value).data, wrap); resetRuntime(); subscribers.at(-1)?.next({ created: { id: 'fresh', group: 'g', title: 'fresh' } }); expect(model.get('fresh')).toMatchObject({ title: 'fresh' }); mounted.unmount();
  });
