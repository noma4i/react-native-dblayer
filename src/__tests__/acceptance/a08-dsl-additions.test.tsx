...
import { belongsTo, bootDb, defineFetch, defineModel, f, flushPersistence, QueryClientProvider, scope, suspendDb } from '../../index';
...
describe(`A08 DSL additions`, () => {
  it(`A08-1 defineFetch happy path is ephemeral and refetchable`, async () => {
    const responses = [{ countries: [{ code: `AU`, name: `Australia` }] }, { countries: [{ code: `AU`, name: `Australia (refetched)` }] }];
  ...
  it(`A08-4 suspendDb flushes pending writes and retains a mounted orphan row`, () => {
    ...
    act(() => {
      ...
      // `orphan` has a null `by`-mapped field, so it never joins any scope.
    ...
    expect(reader.result()).toEqual({ id: `orphan`, group: null, title: `orphan` });
